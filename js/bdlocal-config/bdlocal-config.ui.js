...
function render(){
  ...
  return withTimeout(
    ...
    {
      timeout:true,
      message:"La información secundaria tardó demasiado."
    }
  ).then(function(result){
    fillForms();
    ...
    if(result && result.timeout){
      ...
      if(health){
        health.className = "bdlc-status warning";
        health.textContent = "Carga parcial";
      }

      try{
        console.warn(
          "[BDLocalConfigUI] El resumen excedió 6 segundos. " +
          "La interfaz continuará sin bloquearse."
        );
      }catch(error){}
    }

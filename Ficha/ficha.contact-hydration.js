...
- Evitar cargas concurrentes y ciclos de renderizado repetidos.
========================================================= */
(function(window, document){
  "use strict";

  var VERSION =
    "1.1.0-contact-period-join-stable";
  ...
  function applyContactFields(
    ...
    if(canonicalId){
      target.idEstudiantePeriodo =
        canonicalId;

      if(
        !text(target.studentId) ||
        text(target.studentId) ===
          (
            periodOf(target) +
            "__" +
    ...
    target.__fichaContactHydrationVersion =
      VERSION;
  ...
  function readStore(name){
    ...
    return window.BL2DB
      ...
      .catch(function(error){
        try{
          console.warn(
            "[FichaContactHydration] No se pudo leer " +
            name,
            error
          );
        }catch(innerError){}
  ...
  function ensureCoreReady(){
    ...
    if(
      ...
      typeof window.BDLScreenDepsReady
        .then ===
        "function"
    ){
  ...
  function patchObjectMethod(
    ...
    if(
      original
        .__fichaContactHydrationWrapped
    ){
      return;
    }
  ...
  function refreshView(){
    ...
    window.setTimeout(function(){
      try{
        ...
        try{
          console.error(
            "[FichaContactHydration] No se pudo refrescar la vista",
            error
          );
        }catch(innerError){}
      }

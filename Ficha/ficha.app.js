...
(function(window, document){
  ...
  function detailSignature(row){
    ...
    try{
      reqs =
        window.FichaCore &&
        typeof window.FichaCore.requisitos === "function"
          ? window.FichaCore.requisitos(row)
          : [];
    }catch(error){}
    ...
    try{
      notes =
        window.FichaCore &&
        typeof window.FichaCore.notas === "function"
          ? window.FichaCore.notas(row)
          : [];
    }catch(error2){}
    ...
    return safeJson({
      ...
      matricula:row._estadoMatricula,
      sede:row._sede,
      horario:row._horario,
      correoPersonal:row._correoPersonal,
      correoInstitucional:row._correoInstitucional,
      celular:row._celular,
      periodo:row._periodoId || row.periodoId || row.periodId,
      requisitos:reqs,
      notas:notes,
      modalidad:
        row.modalidad ||
  ...
  function sourceLabel(){
    return (
      window.FichaCore &&
      typeof window.FichaCore.source === "function"
    )
  ...
  function ensureExport(){
    if(
      window.FichaExport &&
      typeof window.FichaExport.copyText ===
        "function"
    ){
      return Promise.resolve();
    }
  ...
  function ensureModalidad(){
    if(
      ...
      typeof window.FichaModalidad.current ===
        "function"
    ){
      return Promise.resolve();
    }
  ...
  function renderList(){
    ...
    if(!state.rows.length){
      box.innerHTML =
        ...
        "Sin estudiantes. Primero carga un Excel en Carga o cambia los filtros." +
        "</div>";
  ...
  function renderReqs(row){
    ...
    try{
      ...
      box.innerHTML =
        reqs.map(function(item){
          return (
            '<div class="ficha-req">' +

              '<span class="ficha-req-name">' +
                esc(item.label) +
              "</span>" +

              '<span class="ficha-req-value ' +
                reqClass(item) +
              '">' +
                esc(
                  item.estado === "cumple"
  ...
  function renderNotas(row){
    ...
    try{
      var notas =
        window.FichaCore.notas
          ? window.FichaCore.notas(row)
          : [];

      box.innerHTML =
        notas.map(function(note){
          var ok =
            note.estado === "cumple";

          return (
            '<article class="ficha-note ' +
              (
                ok
                  ? "ficha-note-ok"
                  : "ficha-note-bad"
  ...
  function applySpecialBadge(id, item){
    ...
    if(!item){
      node.className =
        "ficha-mini-badge ficha-badge-bad";
  ...
  function renderSpecials(row){
    try{
      var list =
        window.FichaCore.especiales
          ? window.FichaCore
              .especiales(row)
          : [];

      applySpecialBadge(
        "ficha-special-titulacion",
  ...
  function periodDisplay(row){
    if(
      window.FichaCore &&
      typeof window.FichaCore.periodDisplay ===
        "function"
    ){
      return window.FichaCore
        .periodDisplay(row);
    }
  ...
  function setModalidadLoading(message){
    var select =
      el("ficha-modalidad-select");

    var info =
      el("ficha-modalidad-info");

    var btn =
      el("ficha-modalidad-save");
    ...
    ensureModalidad()
      ...
      .catch(function(error){
        ...
        if(infoBox){
          infoBox.textContent =
            error.message ||
            "No se pudo cargar modalidad.";

          infoBox.className =
            "ficha-modalidad-info locked";
        }
      });
  }
  ...
  function scheduleModalidad(row){
    if(state.modalidadTimer){
      clearTimeout(
        state.modalidadTimer
      );
    }

    var requestSeq =
      ++state.modalidadRenderSeq;
    ...
    if(!sameStudentVisible){
      setModalidadLoading(
        "Preparando modalidad..."
      );
    }
  ...
  function emailList(row){
    if(
      window.FichaCore &&
      typeof window.FichaCore.emailList ===
        "function"
    ){
      return window.FichaCore
        .emailList(row || {});
  ...
  function updateCommunicationButtons(row){
    ...
    try{
      telegramUrl =
        window.FichaCore &&
        typeof window.FichaCore.telegramUrl ===
          "function"
          ? window.FichaCore
              .telegramUrl(row || {})
          : "";
    ...
    if(telegram){
      ...
      telegram.title =
        telegramUrl
          ? "Copiar mensaje y abrir Telegram"
  ...
  function renderBasicDetail(row){
    ...
    if(estado){
      ...
      estado.className =
        "ficha-pill ficha-pill-warn";
    }

    setText(
      "ficha-division-label",
      row._division ||
      "Sin división"
    );

    setText(
      "ficha-matricula-label",
      row._estadoMatricula ||
      "ACTIVO"
    );
    ...
    setText(
      "ficha-correo-personal",
      row._correoPersonal || "—"
    );

    setText(
      "ficha-correo-institucional",
      row._correoInstitucional || "—"
    );
    ...
    if(modalidadSelect){
      modalidadSelect.removeAttribute(
        "data-ficha-student-id"
      );
    }

    setModalidadLoading(
      "Esperando detalle del estudiante..."
    );
  }
  ...
  function refreshFromBDLocal(){
    ...
    state.refreshTimer =
      setTimeout(function(){
        ...
        if(
          window.FichaCore &&
          typeof window.FichaCore.invalidate ===
            "function"
        ){
  ...
  function openWhatsapp(row){
    ...
    try{
      url =
        window.FichaCore &&
        typeof window.FichaCore.whatsappUrl ===
          "function"
          ? window.FichaCore
              .whatsappUrl(row)
          : "";
  ...
  function openEmail(row){
    ...
    if(!emails.length){
      status(
        "El estudiante no tiene correos registrados.",
        "warn"
      );
    ...
    try{
      url =
        window.FichaCore &&
        typeof window.FichaCore.emailUrl ===
          "function"
          ? window.FichaCore
              .emailUrl(row)
          : "";
    ...
    try{
      ...
      status(
        "No se pudo abrir Outlook. Se copiará el correo.",
        "warn"
      );

      copyText(
        window.FichaCore &&
        window.FichaCore.emailMessage
          ? window.FichaCore
              .emailMessage(row)
          : "",
        "Mensaje de correo copiado."
      );
    }
  }

...
function normalizeFull(row){
  ...
  /*
   * Los requisitos están guardados en requisitos_estudiante
   * como registros separados del estudiante.
   * Se incorporan al objeto usado por Ficha antes de calcular
   * el estado general y los requisitos individuales.
   */
  try{
    var connector =
      window.BDLocalFicha ||
    ...
    if(
      connector &&
      typeof connector.getRequirements === "function"
    ){
      var cedula = text(
        row._cedula ||
        row.cedula ||
      ...
      requirementRows.forEach(function(requirement){
        requirement = requirement || {};

        var value =
          text(requirement.valor) !== ""
            ? requirement.valor
            : requirement.estado;
      ...
      row._requisitos = requirementRows.slice();
    }
  }catch(error){
    try{
      console.warn(
        "[FichaCore] No se pudieron incorporar requisitos de Base Local.",
        error
      );
    }catch(innerError){}
  }

...
- Permitir duración variable por bloque/carrera, usando 30 minutos por defecto.
- Respetar ocupación global aunque se genere con filtros.
- Detectar solapamientos reales de aula y de personas.
========================================================= */
(function(window){
  "use strict";
  ...
  function parseRange(value){var m=txt(value).match(/(\d{1,2}:\d{2})\s*(?:a|hasta|-|–)\s*(\d{1,2}:\d{2})/i);if(!m)return null;var start=parseHora(m[1]),end=parseHora(m[2]);return start!==null&&end!==null&&end>start?{start:start,end:end}:null;}
  function fecha(v){
  ...
  function slots(row,opt){
    ...
    plantillas(row).forEach(function(p){
      (p.bloques||[]).forEach(function(b){
        ...
        var fixedDate=fecha(b.dia),diasBloque=fixedDate?[fixedDate]:dias,ini=parseHora(b.inicio),fin=parseHora(b.fin);
        var dur=Number(b.duracionMinutos||row.duracionMinutos||p.duracionMinutos||DURACION);
        if(!Number.isFinite(dur)||dur<=0)dur=DURACION;
        ...
        diasBloque.forEach(function(d){
          for(var cur=ini;cur+dur<=fin;cur+=dur){
            out.push({
              diaISO:d.iso,dia:d.label,inicio:cur,fin:cur+dur,duracionMinutos:dur,
              aula:txt(b.aula),sede:txt(p.sede||row.sede),
              hora:fmtHora(cur)+"–"+fmtHora(cur+dur),
              tribunal1:t.tribunal1,tribunal2:t.tribunal2,investigador:t.investigador,tribunal3:t.investigador,
              templateId:p.id,tribunalId:b.tribunalId||""
            });
          }
        });
      });
  ...
  function people(row){return [row&&row.tribunal1,row&&row.tribunal2,row&&(row.investigador||row.tribunal3)].map(norm).filter(Boolean);}
  ...
  function conSlot(row,s){
    var r=clone(row);
    r.aula=s.aula;r.dia=s.dia;r.hora=s.hora;r.sede=s.sede||r.sede;r.duracionMinutos=s.duracionMinutos||DURACION;
    r.tribunal1=s.tribunal1;r.tribunal2=s.tribunal2;r.investigador=s.investigador||"";r.tribunal3=r.investigador;
    r.estadoClave="programado";r.estado="Defensa programada";r.cronogramaEstado=r.cronogramaEstado||"BORRADOR";
    r.alertas=Array.isArray(r.alertas)?r.alertas.filter(function(x){return !/Sin cupo|Choque de|Configuración incompleta/i.test(txt(x));}):[];
    r.cronograma={templateId:s.templateId,tribunalId:s.tribunalId,fechaISO:s.diaISO,horaInicio:fmtHora(s.inicio),horaFin:fmtHora(s.fin),duracionMinutos:r.duracionMinutos,generadoEn:new Date().toISOString(),estado:r.cronogramaEstado};
    return r;
  }
  ...
  function detectarConflictos(rows){
    ...
    for(var i=0;i<rows.length;i++){
      if(!tieneHorario(rows[i]))continue;
      for(var j=i+1;j<rows.length;j++){
        if(!tieneHorario(rows[j])||!sameDay(rows[i],rows[j])||!overlap(rows[i],rows[j]))continue;
        if(norm(rows[i].sede)!=="virtual"&&norm(rows[j].sede)!=="virtual"&&txt(rows[i].aula)&&txt(rows[j].aula)&&norm(rows[i].sede)===norm(rows[j].sede)&&norm(rows[i].aula)===norm(rows[j].aula)){
          marcar(i,"Choque de aula en horarios solapados.");marcar(j,"Choque de aula en horarios solapados.");
        }
        var pi=people(rows[i]),pj=people(rows[j]);
        if(pi.some(function(p){return pj.indexOf(p)>=0;})){marcar(i,"Choque de tribunal/investigador en horarios solapados.");marcar(j,"Choque de tribunal/investigador en horarios solapados.");}
      }
    }
    return rows;
  }
  ...
  function generar(rows,opciones){
    opciones=opciones||{};
    var opt={globales:fechas(opciones.diasGlobal||""),porCarrera:fechasPorCarrera(opciones.diasCarrera||"")};
    var sinDias=!opt.globales.length&&!Object.keys(opt.porCarrera).length;
    var occupied=(Array.isArray(opciones.existingRows)?opciones.existingRows:[]).filter(tieneHorario).map(clone);
    (rows||[]).forEach(function(row){if(programable(row)&&tieneHorario(row))occupied.push(clone(row));});
    var out=(rows||[]).map(function(row){
      if(!programable(row))return clone(row);
      if(tieneHorario(row))return clone(row);
      if(sinDias)return sinCupo(row,"No hay días de defensa configurados.");
      var lista=slots(row,opt);
      if(!lista.length)return sinCupo(row,configurationReason(row)||"Configuración incompleta: falta una plantilla o un horario válido.");
      for(var i=0;i<lista.length;i++){
        if(!occupied.some(function(current){return slotConflict(lista[i],current);})){
          var assigned=conSlot(row,lista[i]);occupied.push(clone(assigned));return assigned;
        }
      }
      return sinCupo(row);
    });
  ...
  window.CR_DEF_SCHEDULER=Object.freeze({
    generar:generar,fechas:fechas,fechasPorCarrera:fechasPorCarrera,slots:slots,
    detectarConflictos:detectarConflictos,slotConflict:slotConflict
  });
})(window);

...
(function(window,document){
  ...
  function emptyHtml(message){return '<div class="empty">'+esc(message||"Sin datos.")+'</div>';}
  ...
  function invalidateDataCaches(reason){
    ...
    try{if(window.BL2&&typeof window.BL2.invalidate==="function"){window.BL2.invalidate({emit:false,source:"stats"});}}catch(error){}
    ...
    setTimeout(function(){state.internalInvalidating=false;},0);
  }
  ...
  function renderEstados(data){
    var box=el("stats-estados"), total=data.total||0;if(!box)return;
    if(data._requiresPeriod){box.innerHTML=emptyHtml("Selecciona un período para ver los estados.");setText("stats-estados-meta","0 estudiantes");return;}
  ...
  function renderRequisitos(data){
    ...
    if(data._requiresPeriod){box.innerHTML=emptyHtml("Selecciona un período para calcular requisitos.");return;}
    var rows=data.requisitos||[];
    if(!rows.length){box.innerHTML=emptyHtml("Sin datos.");return;}
  ...
  function table(rows,kind){
    rows=rows||[];
    if(!rows.length)return emptyHtml("Sin datos.");
  ...
  function renderTables(data){setHtml("stats-carreras",data._requiresPeriod?emptyHtml("Selecciona un período."):table(data.carreras,"general"));setHtml("stats-periodos",data._requiresPeriod?emptyHtml("Selecciona un período."):table(data.periodos,"general"));setText("stats-carreras-meta",(data.carreras||[]).length+" carreras");setText("stats-periodos-meta",(data.periodos||[]).length+" períodos");}
  function renderFinales(data){setHtml("stats-finales",data._requiresPeriod?emptyHtml("Selecciona un período."):table(data.requisitosFinales||[],"final"));}

  function renderPeriodApproval(data){
    var chip=el("stats-period-type"), card=el("stats-period-approval-card"), okArticle=el("stats-ok")&&el("stats-ok").closest?el("stats-ok").closest("article"):null, noArticle=el("stats-no")&&el("stats-no").closest?el("stats-no").closest("article"):null, hasPeriod=!!state.periodId;
  ...
  function renderCharts(data){if(data._requiresPeriod){setHtml("stats-chart-general",emptyHtml("Selecciona un período para ver gráficos."));setHtml("stats-chart-selected",emptyHtml("Selecciona un período para ver el requisito seleccionado."));setText("stats-selected-requisito-meta","Todos");return;}if(window.StatsCharts&&typeof window.StatsCharts.renderAll==="function")window.StatsCharts.renderAll(data);}
  function renderNotes(data){if(data._requiresPeriod){setHtml("stats-notes",emptyHtml("Selecciona un período para ver notas."));return;}if(window.StatsNotes&&typeof window.StatsNotes.render==="function")window.StatsNotes.render(data,"stats-notes");}
  function renderStudents(data){if(window.StatsStudents&&typeof window.StatsStudents.render==="function")window.StatsStudents.render(data,"stats-estudiantes",{search:state.studentSearch,limit:data.studentDisplayLimit||150});}
  ...
  function schedulePendingRender(){if(!state.pendingRender)return;var next=state.pendingRender;state.pendingRender=null;setTimeout(function(){render(next);},0);}

  function render(options){
    options=options||{};
    if(state.rendering){state.pendingRender=Object.assign({},state.pendingRender||{},options);return;}
    ...
    try{
      ...
      state.data=window.StatsCore.resumen({periodId:state.periodId,division:state.division,matricula:state.matricula,career:state.career,status:state.status,requirementKey:state.requirementKey,force:options.force===true});
      var data=state.data||{};
      ...
      if(data._requiresPeriod){status("Stats listo. Selecciona un período para cargar estudiantes, requisitos y gráficos.","ok");}
      else{status("Stats cargado por "+source()+". Período: "+(state.periodId||"sin seleccionar")+". Requisito: "+(state.requirementKey||"todos")+".","ok");}
    }catch(error){console.error("[Stats]",error);status(error.message||String(error),"warn");}
    finally{state.rendering=false;schedulePendingRender();}
  }

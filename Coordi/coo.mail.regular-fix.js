/* =========================================================
Nombre completo: coo.mail.regular-fix.js
Ruta: /Coordi/coo.mail.regular-fix.js
Función:
- Mantener intacto el correo PVC que ya funciona bien.
- Adaptar el correo general de períodos Abril-Septiembre al proceso regular de Examen Complexivo.
- Mostrar Modalidad en el resumen y conservar la tabla de cumplimiento por requisito.
========================================================= */
(function(window){
  "use strict";

  var mail=window.COOMail;
  if(!mail||mail.__regularMailPatched||typeof mail.build!=="function"||typeof mail.buildGeneralCompliance!=="function"){return;}

  var VERSION="1.0.0-april-september-complexivo";
  var originalBuild=mail.build.bind(mail);
  var originalGeneral=mail.buildGeneralCompliance.bind(mail);
  ...
  function isAprilSeptember(report){
    var type=report&&report.periodType||{};
    if(text(type.pattern).toUpperCase()==="ABRIL_SEPTIEMBRE"){return true;}
    var value=text(periodLabel(report)).normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase();
    return value.indexOf("abril")>=0&&value.indexOf("septiembre")>=0;
  }
  ...
  function summaryHtml(report,global){
    var cells=[["Período",periodLabel(report)],["Modalidad","Examen Complexivo"],["División",divisionLabel(report)],["Evaluados",fmt(global.totalEstudiantesRevisados)]];
    if(careerLabel(report)){cells.splice(3,0,["Carrera",careerLabel(report)]);}
    return '<table cellpadding="0" cellspacing="0" style="border-collapse:collapse;width:100%;max-width:820px;margin:10px 0 12px;font-family:Arial,sans-serif;font-size:12px;"><tr>'+cells.map(function(cell){return '<td style="border:1px solid #dbe3ee;background:#f8fafc;padding:8px 10px;vertical-align:top;"><span style="display:block;color:#64748b;font-size:10px;font-weight:700;margin-bottom:2px;">'+esc(cell[0])+'</span><strong>'+esc(cell[1])+'</strong></td>';}).join("")+'</tr></table>';
  }

  function complianceHtml(rows){
    rows=arr(rows);
    if(!rows.length){return '<p><strong>Sin datos para mostrar.</strong></p>';}
    var html='<table cellpadding="0" cellspacing="0" style="border-collapse:collapse;width:100%;max-width:820px;font-family:Arial,sans-serif;font-size:12px;color:#0f172a;margin:8px 0 14px;"><thead><tr>';
  ...
  function buildGeneralCompliance(report){
    report=report||{};
    if(!isAprilSeptember(report)){return originalGeneral(report);}

    var base=originalGeneral(report)||{};
    var global=report.global||{};
    var rows=arr(report.compliance||global.cumplimiento);
    var saludo=text(global.saludo||"Estimadas autoridades y responsables de área");
    var career=careerLabel(report);

    var html='<div style="font-family:Arial,sans-serif;color:#0f172a;font-size:13px;line-height:1.45;max-width:840px;">'+
      '<p style="margin:0 0 10px;"><strong>'+esc(saludo)+':</strong></p>'+
      '<p style="margin:0 0 10px;">Reciban un cordial saludo.</p>'+
      '<p style="margin:0 0 10px;">Remito el <strong>reporte general de cumplimiento de requisitos</strong> de los estudiantes del proceso de titulación correspondiente al <strong>Examen Complexivo</strong>, período <strong>'+esc(periodLabel(report))+'</strong>.</p>'+
      summaryHtml(report,global)+
      '<p style="margin:10px 0 6px;font-weight:700;color:#1e3a8a;">Estado de cumplimiento por requisito</p>'+
      complianceHtml(rows)+
      '<p style="margin:0 0 10px;">Se solicita a las áreas correspondientes realizar el <strong>seguimiento prioritario de los casos pendientes</strong>, con el fin de completar los requisitos habilitantes y garantizar que los estudiantes puedan continuar con las etapas correspondientes al <strong>Examen Complexivo</strong>.</p>'+
      '<p style="margin:0 0 10px;">Agradezco de antemano su atención y gestión oportuna.</p>'+signatureHtml()+'</div>';

    var plain=[
      saludo+":","","Reciban un cordial saludo.","",
      "Remito el reporte general de cumplimiento de requisitos de los estudiantes del proceso de titulación correspondiente al Examen Complexivo, período "+periodLabel(report)+".","",
      "RESUMEN GENERAL","Período: "+periodLabel(report),"Modalidad: Examen Complexivo","División: "+divisionLabel(report)
    ];
    if(career){plain.push("Carrera: "+career);}
    plain=plain.concat([
      "Evaluados: "+fmt(global.totalEstudiantesRevisados),"","ESTADO DE CUMPLIMIENTO POR REQUISITO",plainTable(rows),"",
      "Se solicita a las áreas correspondientes realizar el seguimiento prioritario de los casos pendientes, con el fin de completar los requisitos habilitantes y garantizar que los estudiantes puedan continuar con las etapas correspondientes al Examen Complexivo.","",
      "Agradezco de antemano su atención y gestión oportuna.","",signaturePlain()
    ]);
  ...
  mail.build=function(report,options){
    options=options||{};
    if((options.kind==="global"||options.kind==="general")&&isAprilSeptember(report)){return buildGeneralCompliance(report);}
    return originalBuild(report,options);
  };
  mail.__regularMailPatched=true;
  mail.regularMailVersion=VERSION;
  window.COOMailRegularFix={version:VERSION,isAprilSeptember:isAprilSeptember,buildGeneralCompliance:buildGeneralCompliance};
})(window);

...
(function(window,document){
  ...
  function clearStatusTimer(){ if(state.statusTimer){ clearTimeout(state.statusTimer); state.statusTimer = null; } }
  ...
  function requestSnapshot(options){
    return {periodId:state.periodId,division:state.division,career:state.career,requirementKey:state.requirementKey,refresh:options && options.refresh === true};
  }
  function requestIsCurrent(request){
    return request.periodId === state.periodId && request.division === state.division && request.career === state.career && request.requirementKey === state.requirementKey;
  }
  ...
  function render(options){
    ...
    if(state.loading){ state.pendingRender = mergeRenderOptions(state.pendingRender,options); return Promise.resolve(null); }
    state.loading = true;
  ...
  function scheduleDataRefresh(){
    ...
    state.refreshTimer = setTimeout(function(){ state.refreshTimer = null; render({refresh:false}); },260);
  }
  ...
  function bindStatic(){
    ...
    if(periodo){ periodo.addEventListener("change",function(event){ state.periodId = event.target.value; state.division = ""; state.career = ""; state.requirementKey = ""; render(); }); }
    if(division){ division.addEventListener("change",function(event){ state.division = event.target.value; state.career = ""; state.requirementKey = ""; render(); }); }
    if(carrera){ carrera.addEventListener("change",function(event){ state.career = event.target.value; state.requirementKey = ""; render(); }); }
    if(requisito){ requisito.addEventListener("change",function(event){ state.requirementKey = event.target.value; render(); }); }

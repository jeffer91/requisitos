...
Función o funciones:
- Orquestar la pantalla Infor desde la carpeta definitiva /Requisitos/Infor.
...
- Actualizar pasos visuales, revisión QA en vivo y validación previa a procesar.
Con qué se conecta:
...
(function(window, document){
  ...
  var state = {periods:[], anexos:[], anexoPeriodKey:"", booted:false, periodSummary:null};
  ...
  function snapshotPeriodKey(snapshot){snapshot = snapshot || {};return text(snapshot.periodId || snapshot.periodLabel || "SIN_PERIODO");}
  ...
  function updateFlow(snapshot){
    var steps = Array.prototype.slice.call(document.querySelectorAll(".infor-flow .infor-step"));
    ...
    var hasInsumos = excelRows > 0 || !!text(snapshot.cronogramas && (snapshot.cronogramas.complexivo || snapshot.cronogramas.trabajoTitulacion || snapshot.cronogramas.pvc)) || (Array.isArray(snapshot.anexos) && snapshot.anexos.length > 0);
    var reportReady = !!(snapshot.reportDraft && snapshot.reportDraft.ok);
    var stateMap = [
      {active:!hasPeriod, done:hasPeriod},
      {active:hasPeriod && !reportReady, done:hasInsumos && !!snapshot.matchResult},
      {active:reportReady, done:reportReady}
    ];
    steps.forEach(function(step, index){
      var item = stateMap[index] || {active:false,done:false};
      step.classList.toggle("is-active", !!item.active && !item.done);
      step.classList.toggle("is-done", !!item.done);
    });
  }
  ...
  function readiness(snapshot){
    ...
    var hasPeriod = !!text(snapshot.periodId || snapshot.periodLabel);
    var excelRows = snapshot.excelData && Array.isArray(snapshot.excelData.rows) ? snapshot.excelData.rows.length : 0;
    var hasGemini = !!(window.InforState && window.InforState.getGeminiKey && window.InforState.getGeminiKey());
    if(!hasPeriod){missing.push("seleccionar período");}
    if(!excelRows){missing.push("cargar Excel con estudiantes");}
    if(!hasGemini){missing.push("configurar Gemini");}
    return {ready:missing.length === 0, missing:missing, message:missing.length ? ("Falta: " + missing.join(", ") + ".") : "Listo para procesar el informe."};
  }

  function updateProcessReadiness(snapshot){
    var node = el("infor-process-readiness");
    var btn = el("infor-process");
    var info = readiness(snapshot);
    if(node){node.textContent = info.message;node.className = "infor-muted " + (info.ready ? "ok" : "warn");}
    if(btn){btn.title = info.message;}
    return info;
  }
  ...
  function smallTable(headers, rows){
    ...
    html += rows.map(function(row){
      return '<tr>' + headers.map(function(h){var value = typeof h.value === "function" ? h.value(row) : row[h.key];return '<td>' + value + '</td>';}).join("") + '</tr>';
    }).join("");
  ...
  function renderMatch(snapshot){
    ...
    var box = el("infor-match-preview");
    if(!box){return;}
  ...
  function setExportButtons(ready){
    var w = el("infor-export-word"), p = el("infor-export-pdf");
    if(w){w.disabled = !ready;}
  ...
  function renderReport(snapshot){
    ...
    var ready = !!(report && report.ok);
    setExportButtons(ready);
    setChip("infor-report-state", ready ? "Motor listo" : "Sin motor", ready ? "ok" : "warn");
    ...
    var box = el("infor-report-preview");
    if(!box){return;}
    if(!ready){box.innerHTML = '<div class="infor-empty">' + esc(statusInfo.message || 'Presiona Procesar para construir el motor del informe.') + '</div>';return;}
    box.innerHTML = smallTable([
      ...
      {label:"Estado", value:function(){return '<span class="infor-pill-mini ok">Lista</span>';}}
    ], report.sections || []);
  }
  ...
  function syncAnexosFromSnapshot(snapshot){
    snapshot = snapshot || window.InforState.getState();
    var key = snapshotPeriodKey(snapshot);
    if(state.anexoPeriodKey === key){return;}
    state.anexoPeriodKey = key;
    state.anexos = (Array.isArray(snapshot.anexos) ? snapshot.anexos : []).map(function(item){
      return {name:item.name || "", size:item.size || 0, type:item.type || "", title:item.title || item.name || "", createdAt:item.createdAt || "", preview:"", persisted:true};
    });
    var input = el("infor-anexos-input");
  ...
  function renderAnexos(){
    ...
    box.innerHTML = state.anexos.map(function(item, index){
      var preview = item.preview ? '<img src="' + esc(item.preview) + '" alt="' + esc(item.title || item.name) + '">' : '<div class="infor-empty">Vista previa no disponible en esta sesión.</div>';
      return '<article class="infor-anexo" data-index="' + index + '">' + preview + '<input class="infor-anexo-title" type="text" data-title-index="' + index + '" value="' + esc(item.title || '') + '" placeholder="Título del anexo" />' + '<div class="infor-anexo-meta"><span>' + esc(item.name || 'imagen') + '</span><span>' + Math.round((item.size || 0)/1024) + ' KB</span></div></article>';
    }).join("");
  }
  ...
  function renderDiagnostics(){
    ...
    node.textContent = JSON.stringify({
      bloque:"Bloque de corrección y búsqueda de errores Infor",
      ...
      matchSummary:snapshot.matchResult && snapshot.matchResult.summary,
      reportStatus:snapshot.reportStatus,
      reportResumen:snapshot.reportDraft && snapshot.reportDraft.resumen,
      reportKind:snapshot.reportDraft && snapshot.reportDraft.kind,
      reportSections:snapshot.reportDraft && snapshot.reportDraft.sections ? snapshot.reportDraft.sections.length : 0,
      geminiAnalysis:!!snapshot.geminiAnalysis,
      cronogramasParsed:snapshot.cronogramasParsed,
      anexos:state.anexos.map(function(x){return {name:x.name,title:x.title,size:x.size,type:x.type,persisted:!!x.persisted,hasPreview:!!x.preview};}),
  ...
  function renderQA(){
    try{if(window.InforQA && typeof window.InforQA.run === "function" && typeof window.InforQA.render === "function"){window.InforQA.render(window.InforQA.run());}}
    catch(error){console.warn("[Infor QA render]", error);}
  }
  ...
  function renderAll(message, cls){
    ...
    renderAnexos();
    updateProcessReadiness(snapshot);
    renderDiagnostics();
    renderQA();
  ...
  function onPeriodChange(){
    ...
    if(!id){
      ...
      syncAnexosFromSnapshot(window.InforState.getState());
      renderAll("Selecciona un período para iniciar.", "warn");
      return;
    }
    window.InforState.loadPeriod(id, label);
    if((window.InforState.getState().excelData.rows || []).length){runMatch();}
    syncAnexosFromSnapshot(window.InforState.getState());
  ...
  async function onExcelChange(event){
    ...
    try{
      ...
      runMatch();
      renderAll("Excel leído y unido: " + (analysis.usefulSheets || 0) + " hojas útiles, " + (analysis.totalRows || 0) + " filas detectadas.", "ok");
    }catch(error){
  ...
  function onCronogramaInput(kind, value){
    ...
    updateProcessReadiness(window.InforState.getState());
    renderDiagnostics();
    renderQA();
  }
  ...
  function onAnexosChange(event){
    ...
    var mapped = files.filter(function(file){return /^image\//.test(file.type || "");}).map(function(file){
      return {name:file.name,size:file.size,type:file.type,title:file.name,createdAt:new Date().toISOString(),preview:URL.createObjectURL(file),persisted:false};
    });
    state.anexoPeriodKey = snapshotPeriodKey(window.InforState.getState());
  ...
  async function process(){
    var snapshot = window.InforState.getState();
    var gate = readiness(snapshot);
    if(!gate.ready){renderAll(gate.message, "warn");return;}
    try{
      ...
      renderAll("Informe listo para exportar en Word o PDF.", "ok");
    }catch(error){
  ...
  function currentReport(){
    var snapshot = window.InforState.getState();
    var report = snapshot.reportDraft || null;
    if(!(report && report.ok)){throw new Error("Primero procesa el informe correctamente.");}
    return report;
  }

  function exportWord(){
    try{
      if(!(window.InforWordExport && typeof window.InforWordExport.download === "function")){throw new Error("InforWordExport no está disponible.");}
      var result = window.InforWordExport.download(currentReport(), state.anexos);
      status("Word generado: " + result.filename + ".", "ok");
    }catch(error){console.error("[Infor Word]", error);status(error.message || String(error), "bad");}
  }

  function exportPdf(){
    try{
      if(!(window.InforPdfExport && typeof window.InforPdfExport.print === "function")){throw new Error("InforPdfExport no está disponible.");}
      window.InforPdfExport.print(currentReport(), state.anexos);
      status("Vista PDF abierta. Guarda como PDF desde la ventana de impresión.", "ok");
    }catch(error){console.error("[Infor PDF]", error);status(error.message || String(error), "bad");}
  }
  ...
  function bindEvents(){
    ...
    bind("infor-anexos-list", "input", function(e){
      ...
      if(index >= 0 && state.anexos[index]){state.anexos[index].title = e.target.value;saveAnexosToState();renderDiagnostics();renderQA();}
    });
    ...
    bind("infor-export-word", "click", exportWord);
    bind("infor-export-pdf", "click", exportPdf);
  ...
  function boot(){
    try{
      ...
      state.booted = true;
    }catch(error){console.error("[Infor boot]", error);status(error.message || String(error), "bad");}
  }

  if(document.readyState === "loading"){document.addEventListener("DOMContentLoaded", boot);}else{boot();}
  window.InforApp = {render:renderAll,getState:function(){return Object.assign({}, state);},runMatch:runMatch,process:process,exportWord:exportWord,exportPdf:exportPdf,syncAnexosFromSnapshot:syncAnexosFromSnapshot};
})(window, document);

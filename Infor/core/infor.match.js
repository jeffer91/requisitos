...
Función o funciones:
- Unir filas del Excel de Infor con estudiantes de BaseLocal/BL2 sin reemplazar la fuente principal del informe.
...
- Priorizar modalidad, notas y datos académicos provenientes del Excel.
- En períodos regulares, usar InforRegular para preparar NÚCLEOS y notas_complexivo.
Con qué se conecta:
- infor.state.js
- infor.periodo.js
- infor.regular.js
...
(function(window){
  ...
  function baseCedula(row){row = row || {};return text(row._cedula || row._bl2Id || row.cedula || row.Cedula || row.CEDULA || row.numeroIdentificacion || row.identificacion || row.docId || row._docId || findValue(row, CEDULA_ALIASES));}
  function baseName(row){row = row || {};return text(row._nombres || row._bl2Nombre || row.nombres || row.Nombres || row.nombre || row.nombre_est || row.estudiante || findValue(row, NAME_ALIASES));}
  function baseCareer(row){row = row || {};return text(row._carrera || row._bl2Carrera || row.nombreCarrera || row.nombrecarrera || row.carrera || row.Carrera || findValue(row, CAREER_ALIASES));}
  ...
  function normalizeBaseStudent(row, periodType){
    ...
    return {id:text(row._id || row._docId || row.docId || baseCedula(row)),cedula:baseCedula(row),nombres:baseName(row),carrera:baseCareer(row),periodo:text(row._periodo || row.periodoLabel || row.periodoId || row.ultimoPeriodoId || row.periodo || ""),modalidadTitulacion:modalidad,modalidadLabel:modalityLabel(modalidad),raw:row};
  }
  ...
  function normalizeExcelRow(row, periodType){
    ...
    var modalidadRaw = text(row.modalidadTitulacion || row.ModalidadTitulacion || findValue(row, MODALIDAD_ALIASES));
    var modalidad = normalizeModality(modalidadRaw, periodType);
    return {sheet:text(row._inforSheet || ""),rowNumber:row._inforRowNumber || "",cedula:text(row.cedula || findValue(row, CEDULA_ALIASES)),nombres:text(row.nombres || findValue(row, NAME_ALIASES)),carrera:text(row.carrera || findValue(row, CAREER_ALIASES)),titulo:text(row.titulo || findValue(row, TITLE_ALIASES)),tutor:text(row.tutor || findValue(row, TUTOR_ALIASES)),modalidadTitulacion:modalidad,modalidadLabel:modalityLabel(modalidad),raw:row};
  }
  ...
  function matchExcelRow(excel, index, periodType){
    var byCedula = null;
    cedulaVariants(excel.cedula).some(function(key){if(index.byCedula[key]){byCedula = index.byCedula[key];return true;}return false;});
  ...
  function buildMatch(method, excel, base, periodType){
    ...
    return {status:base ? "unido" : "pendiente",method:method,cedula:excel.cedula || (base && base.cedula) || "",nombres:excel.nombres || (base && base.nombres) || "",carrera:excel.carrera || (base && base.carrera) || "",titulo:excel.titulo || "",tutor:excel.tutor || "",modalidadTitulacion:modalidad,modalidadLabel:modalityLabel(modalidad),excel:excel,base:base};
  }
  ...
  function summarize(matches, baseSource, baseTotal, regularAnalysis){
    ...
    if(regularAnalysis && regularAnalysis.summary){out.regular = regularAnalysis.summary;out.excluidosPeriodo = 0;out.duplicadosOmitidos = regularAnalysis.summary.duplicates || 0;out.inconsistencias = regularAnalysis.summary.inconsistencias || 0;out.supletorios = regularAnalysis.summary.supletorios || 0;out.retirados = regularAnalysis.summary.retirados || 0;}
    return out;
  }
  ...
  function match(snapshot){
    ...
    return {ok:excelRows.length > 0,periodId:periodId,periodType:periodType,baseSource:base.source,baseTotal:index.rows.length,totalExcel:excelRows.length,totalExcelOriginal:sourceRows.length,regularAnalysis:regularAnalysis,matches:matches,summary:summarize(matches, base.source, index.rows.length, regularAnalysis),generatedAt:new Date().toISOString()};
  }

  window.InforMatch = {match:match,normalizeModality:normalizeModality,modalityLabel:modalityLabel,cedulaVariants:cedulaVariants,nameKey:nameKey};
})(window);

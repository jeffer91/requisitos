...
- Fusionar el registro antiguo con cualquier destino canónico existente.
...
(function(window,document){
  ...
  var VERSION="1.1.0-safe-destination-merge";
  ...
  function normalizeKey(value){
    var utils=config().utils||{};
    return typeof utils.normalizeKey==="function"
  ...
  function descriptors(){
    ...
    return list.filter(function(item,index){return item.name&&list.findIndex(function(other){return other.name===item.name;})===index;});
  }
  ...
  function readAll(){
    ...
    return Promise.all(descriptors().map(function(descriptor){
      return current.getAll(descriptor.name).catch(function(){return [];}).then(function(rows){
        return {descriptor:descriptor,rows:Array.isArray(rows)?rows:[]};
      });
    }));
  }
  ...
  function suffixFromId(value,bases){
    ...
    for(var i=0;i<bases.length;i+=1){
      var base=text(bases[i]);
      if(base&&value.indexOf(base+"__")===0){return value.slice((base+"__").length);}
    }
    return "";
  }
  ...
  function transformedKey(row,descriptor,canonicalCedula,periodoId){
    ...
    if(["student","note","noteLegacy","contact"].indexOf(descriptor.kind)>=0){return base||text(row[descriptor.keyPath]);}
    ...
    if(descriptor.kind==="division"){
      var oldCedula=rawCedula(row);
      var oldCanonical=normalizeCedula(oldCedula);
      var oldBases=[
        text(row.idEstudiantePeriodo||row.studentId),
        periodoId&&oldCedula?oldCedula+"__"+periodoId:"",
        periodoId&&oldCedula?periodoId+"__"+oldCedula:"",
        periodoId&&oldCanonical?periodoId+"__"+oldCanonical:""
      ];
  ...
  function replaceReferenceString(value,info){
    ...
    for(var i=0;i<info.oldStudentIds.length;i+=1){
      var oldId=info.oldStudentIds[i];
      if(value===oldId){return info.canonicalStudentId;}
      if(oldId&&value.indexOf(oldId+"__")===0){return info.canonicalStudentId+value.slice(oldId.length);}
    }
    return value;
  }
  ...
  function transformRow(row,descriptor){
    ...
    var raw=rawCedula(original);
    var canonical=normalizeCedula(raw);
    var periodoId=rowPeriod(original);
    if(!canonical){return null;}

    var canonicalStudent=studentId(canonical,periodoId);
    var oldStudentIds=[];
    [
      original.idEstudiantePeriodo,
      original.studentId,
      periodoId&&raw?raw+"__"+periodoId:"",
      periodoId&&raw?periodoId+"__"+raw:"",
      periodoId&&canonical?periodoId+"__"+canonical:""
    ].forEach(function(value){value=text(value);if(value&&oldStudentIds.indexOf(value)<0){oldStudentIds.push(value);}});

    var info={
      rawCedula:raw,
      cleanedRawCedula:text(raw).replace(/[^0-9A-Za-z]/g,"").toUpperCase(),
      canonicalCedula:canonical,
      periodoId:periodoId,
      canonicalStudentId:canonicalStudent,
      oldStudentIds:oldStudentIds
    };

    var output=descriptor.kind==="reference"?patchReferences(original,info,0):Object.assign({},original);
    ...
    if(descriptor.kind!=="reference"){
      ...
      if(periodoId){
        output.periodoId=periodoId;
        if("periodId" in output){output.periodId=periodoId;}
        if("periodoCanonicoId" in output){output.periodoCanonicoId=periodoId;}
      }
      if(canonicalStudent&&descriptor.kind!=="person"){
        output.idEstudiantePeriodo=canonicalStudent;
        output.studentId=canonicalStudent;
      }
    }

    var newKey=transformedKey(output,descriptor,canonical,periodoId);
    var oldKey=text(original[descriptor.keyPath]);

    if(descriptor.kind!=="reference"){
      output[descriptor.keyPath]=newKey;
      if(["student","note","noteLegacy","contact"].indexOf(descriptor.kind)>=0&&("id" in output||descriptor.keyPath==="id")){output.id=newKey;}
      if(descriptor.kind==="note"||descriptor.kind==="noteLegacy"){output.notaId=newKey;output.id=newKey;}
    }

    var changed=JSON.stringify(original)!==JSON.stringify(output);
    if(changed){
      output.identityCanonicalizedAt=now();
      output.identityCanonicalizationSource="BDLocal.MantenimientoSeguro";
      output.updatedAt=text(original.updatedAt)||now();
    }
    ...
    return {
      ...
      changed:changed,
      analysis:analyzeIdentification(raw)
    };
  }
  ...
  function criticalConflict(rows){
    ...
    rows.forEach(function(row){
      var name=normalizedName(row);
      var user=normalizedTelegramUser(row);
      var chatId=normalizedTelegramChat(row);
      if(name){names[name]=true;}
      if(user){users[user]=true;}
      if(chatId){chats[chatId]=true;}
    });
    var reasons=[];
  ...
  function mergeNonEmpty(rows){
    ...
    rows.forEach(function(row){
      Object.keys(row||{}).forEach(function(name){
        ...
        if(value===undefined||value===null||text(value)===""){
          if(output[name]===undefined){output[name]=clone(value);}
        }else{
    ...
    var newest=rows.reduce(function(latest,row){return timestamp(row)>timestamp(latest)?row:latest;},rows[0]||{});
    output.updatedAt=text(newest.updatedAt)||now();
    return output;
  }
  ...
  function uniqueItems(items){
    ...
    return (items||[]).filter(function(item){
      var key=item.descriptor.name+"::"+item.oldKey+"::"+item.newKey;
  ...
  function buildPlan(results){
    ...
    results.forEach(function(result){
      result.rows.forEach(function(row){
        var item=transformRow(row,result.descriptor);
        if(item){transformed.push(item);}
      });
    });

    var changed=transformed.filter(function(item){return item.changed;});
    var changedIdentities={};
    changed.forEach(function(item){changedIdentities[item.identityKey]=true;});

    var groups={};
    transformed.forEach(function(item){
      if(!changedIdentities[item.identityKey]){return;}
      if(!groups[item.identityKey]){groups[item.identityKey]={identity:item.identityKey,allItems:[],changedItems:[],conflicts:[]};}
      groups[item.identityKey].allItems.push(item);
      if(item.changed){groups[item.identityKey].changedItems.push(item);}
    });
    ...
    Object.keys(groups).sort().forEach(function(identity){
      ...
      var criticalRows=group.allItems.filter(function(item){return ["person","student","contact"].indexOf(item.descriptor.kind)>=0;}).map(function(item){return item.original;});
      var reasons=criticalConflict(criticalRows);
      if(reasons.length){group.conflicts=reasons;conflictGroups.push(group);return;}

      var buckets={};
      group.changedItems.forEach(function(item){
        var bucketKey=item.descriptor.name+"::"+item.newKey;
        if(!buckets[bucketKey]){buckets[bucketKey]={descriptor:item.descriptor,newKey:item.newKey,changedItems:[]};}
        buckets[bucketKey].changedItems.push(item);
      });
      ...
      Object.keys(buckets).forEach(function(bucketKey){
        ...
        if(bucket.descriptor.kind==="reference"){
          bucket.changedItems.forEach(function(item){
            group.operations.push({
              store:bucket.descriptor.name,
              keyPath:bucket.descriptor.keyPath,
              newKey:item.oldKey,
              put:item.row,
              deletes:[],
        ...
        var contextItems=group.allItems.filter(function(item){
          return item.descriptor.name===bucket.descriptor.name&&item.newKey===bucket.newKey;
        });
        contextItems=uniqueItems(contextItems.concat(bucket.changedItems));

        var merged=mergeNonEmpty(contextItems.map(function(item){return item.row;}));
        merged[bucket.descriptor.keyPath]=bucket.newKey;

        var deletes=bucket.changedItems.map(function(item){return item.oldKey;}).filter(function(value,index,list){
          return value&&value!==bucket.newKey&&list.indexOf(value)===index;
        });

        group.operations.push({
          store:bucket.descriptor.name,
          keyPath:bucket.descriptor.keyPath,
          newKey:bucket.newKey,
          put:merged,
          deletes:deletes,
          sources:contextItems.map(function(item){return item.original;})
        });
      });
    ...
    return {
      ...
      deletes:selected.reduce(function(total,group){
        return total+group.operations.reduce(function(sum,operation){return sum+operation.deletes.length;},0);
      },0),
      selected:selected,
  ...
  function publicPlan(plan){
    ...
    return {
      ...
      identitiesFound:plan.identitiesFound,
      safeIdentities:plan.safeIdentities,
      conflictIdentities:plan.conflictIdentities,
      selectedIdentities:plan.selectedIdentities,
      remainingSafeIdentities:plan.remainingSafeIdentities,
      operations:plan.operations,
      deletes:plan.deletes,
      selected:plan.selected.map(function(group){
        return {
          identity:group.identity,
          operations:group.operations.map(function(operation){
            return {
              store:operation.store,
              newKey:operation.newKey,
              deletes:operation.deletes,
              sourceRows:operation.sources.length
            };
          })
        };
      }),
      conflicts:plan.conflicts.slice(0,50).map(function(group){
        return {identity:group.identity,reasons:group.conflicts,rows:group.allItems.length};
      })
    };
  }
  ...
  function preview(){
    ...
    running=true;
    lastPlan=null;
    status("Analizando identidades y relaciones locales...","info");

    return readAll().then(function(results){
      var plan=buildPlan(results);
      lastPlan=plan;
      lastPreview=Object.assign({
        ...
        canonicalLocalId:"cedula__periodoId",
        writesExternal:false,
        createsOutbox:false,
        message:"Análisis local: "+plan.safeIdentities+" identidad(es) segura(s), "+plan.conflictIdentities+" conflicto(s), "+plan.selectedIdentities+" seleccionada(s) para el siguiente lote."
      },publicPlan(plan));
      render(lastPreview);
      status(lastPreview.message,plan.conflictIdentities?"warning":"success");
  ...
  function backupPlan(plan){
    ...
    var documents=[];
    plan.selected.forEach(function(group){
      group.operations.forEach(function(operation){
        documents.push({
          ...
          merged:operation.put
        });
      });
    });
    return repository.save({
      scope:"bdlocal.identity",
      tipo:"pre_local_identity_repair",
      type:"pre_local_identity_repair",
      schemaVersion:"2",
      totalRegistros:documents.length,
      origen:"BDLLocalIdentityRepair",
      payload:{createdAt:now(),canonicalLocalId:"cedula__periodoId",documents:documents}
    });
  }
  ...
  function executeTransaction(plan){
    ...
    plan.selected.forEach(function(group){
      group.operations.forEach(function(operation){
        if(storeNames.indexOf(operation.store)<0){storeNames.push(operation.store);}
      });
    });
    ...
    return current.tx(storeNames,"readwrite").then(function(transaction){
      return new Promise(function(resolve,reject){
        ...
        transaction.oncomplete=function(){if(!settled){settled=true;resolve({puts:puts,deletes:deletes});}};
        transaction.onerror=function(){if(!settled){settled=true;reject(transaction.error||new Error("No se pudo consolidar Base Local."));}};
        transaction.onabort=function(){if(!settled){settled=true;reject(transaction.error||new Error("La consolidación local fue cancelada por IndexedDB."));}};

        try{
          plan.selected.forEach(function(group){
            group.operations.forEach(function(operation){
              var objectStore=transaction.objectStore(operation.store);
              objectStore.put(clone(operation.put));
              puts+=1;
              operation.deletes.forEach(function(oldKey){objectStore.delete(oldKey);deletes+=1;});
            });
          });
        }catch(error){
          try{transaction.abort();}catch(inner){}
  ...
  function refresh(){
    ...
    return clear.then(function(){
      try{
        window.dispatchEvent(new CustomEvent("bdlocal:identity-local-repaired",{
          detail:{at:now(),canonicalLocalId:"cedula__periodoId",queued:false}
        }));
      }catch(error){}
      var hub=window.BDLocalConexiones;
      return hub&&typeof hub.refreshCache==="function"
        ?hub.refreshCache({force:true,light:true,source:"local_identity_repair"}).catch(function(){return null;})
        :null;
    });
  }
  ...
  function apply(){
    ...
    if(!window.confirm("Consolidar identidades en Base Local\n\nIdentidades: "+plan.selectedIdentities+"\nOperaciones: "+plan.operations+"\nClaves antiguas a retirar: "+plan.deletes+"\n\nSe creará un respaldo local y no se enviará nada a Firebase ni Google. ¿Continuar?")){
      return Promise.resolve({ok:true,cancelled:true,message:"Consolidación cancelada."});
    }
    if(running){return Promise.resolve({ok:true,skipped:true,message:"Ya existe una consolidación local en curso."});}

    running=true;
    status("Creando respaldo antes de consolidar...","warning");

    return backupPlan(plan)
      .then(function(backup){return executeTransaction(plan).then(function(result){return {backup:backup,result:result};});})
      .then(function(context){return refresh().then(function(){return context;});})
      .then(function(context){
        var result={
          ok:true,
          identities:plan.selectedIdentities,
          operations:context.result.puts,
          deletes:context.result.deletes,
          remainingSafeIdentities:plan.remainingSafeIdentities,
          conflicts:plan.conflictIdentities,
          backupId:context.backup&&(context.backup.id||context.backup.backupId)||"",
          writesExternal:false,
          createsOutbox:false,
          message:"Base Local consolidada: "+plan.selectedIdentities+" identidad(es), "+context.result.puts+" registro(s) guardado(s) y "+context.result.deletes+" clave(s) antigua(s) retirada(s)."
        };
        lastPlan=null;
        lastPreview=result;
        render(result);
      ...
      .catch(function(error){status(error.message||String(error),"error");throw error;})
      .finally(function(){running=false;render(lastPreview||{});});
  }

  function status(message,type){
    var node=id("local-identity-status");
    if(node){node.className="bdlc-alert "+(type||"info");node.textContent=message;}
  }

  function render(report){
    var summary=id("local-identity-summary");
    var output=id("local-identity-json");
    var button=id("local-identity-apply");
    if(!summary||!output){return;}
    report=report||{};
    var rows=[
      ["Filas revisadas",Number(report.scannedRows||0)],
      ["Filas que requieren ajuste",Number(report.affectedRows||0)],
      ["Identidades seguras",Number(report.safeIdentities||report.identities||0)],
      ["Identidades con conflicto",Number(report.conflictIdentities||report.conflicts||0)],
      ["Seleccionadas en este lote",Number(report.selectedIdentities||report.identities||0)],
      ["Pendientes para otro lote",Number(report.remainingSafeIdentities||0)],
      ["Claves antiguas",Number(report.deletes||0)]
    ];
  ...
  function mount(container){
    ...
    if(!card){
      ...
      card.id="local-identity-repair-card";
  ...
  window.BDLLocalIdentityRepair={
    ...
    buildPlan:buildPlan,
    getLastPreview:function(){return clone(lastPreview);},
    writesExternal:false,
    createsOutbox:false
  };

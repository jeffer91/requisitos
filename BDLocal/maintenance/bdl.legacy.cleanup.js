...
- Usar cedula__periodoId como clave local canónica.
...
(function(window,document){
  ...
  var VERSION="0.4.0-canonical-local-id";
  ...
  function currentPeriod(){
    if(window.BL2App&&typeof window.BL2App.getSelectedPeriod==="function"){
      ...
      if(selected&&text(selected.id)){return {id:canonicalPeriodId(selected.id),label:text(selected.label||selected.id)};}
    }
    var select=id("bl2-period-select");
    var periodoId=canonicalPeriodId(select&&select.value);
  ...
  function rowPeriod(row){return canonicalPeriodId(row&&(row.periodoId||row.periodId||row.periodoCanonicoId));}
  function rowCedula(row){return normalizeCedula(row&&(row.cedula||row.numeroIdentificacion||row.NumeroIdentificacion||row.idPersona));}
  function rowStudentId(row){
    var periodoId=rowPeriod(row);
    var cedula=rowCedula(row);
    if(periodoId&&cedula){return cedula+"__"+periodoId;}
    return text(row&&(row.idEstudiantePeriodo||row.studentId||row.id));
  }
  function requirementRelationKey(row){
    var base=rowStudentId(row);
    var requirement=normalizeKey(row&&(row.requisitoKey||row.key||row.nombre||row.requisitoLabel));
    return base&&requirement?base+"__"+requirement:"";
  }
  ...
  function expectedStores(){
    var declared=config().stores||{};
    var seen=Object.create(null);
    return Object.keys(declared).map(function(key){return text(declared[key]);}).filter(function(name){if(!name||seen[name]){return false;}seen[name]=true;return true;});
  }
  ...
  function expectedIndexes(){
    ...
    Object.keys(source).forEach(function(key){
      var table=text(declared[key]||key);
  ...
  function inspectIndexes(){
    ...
    return current.open().then(function(nativeDb){
      ...
      Object.keys(expected).forEach(function(table){
        ...
        var objectStore=nativeDb.transaction([table],"readonly").objectStore(table);
        var actual=Array.prototype.slice.call(objectStore.indexNames||[]);
        var missing=expected[table].filter(function(index){return actual.indexOf(index)<0;});
        result.tables[table]={exists:true,keyPath:Array.isArray(objectStore.keyPath)?objectStore.keyPath.join(" + "):text(objectStore.keyPath),expected:expected[table],actual:actual,missing:missing};
  ...
  function missingRelation(rows,valid){
    return (rows||[]).filter(function(row){var key=rowStudentId(row);return !!key&&!valid[key];});
  }
  ...
  function analyzeBlocked(changes){
    ...
    ["google","firebase","supabase"].forEach(function(target){
      (changes||[]).forEach(function(row){
        var blocked=queue&&typeof queue.isBlocked==="function"?queue.isBlocked(row,target,{}):false;
        if(blocked){result[target]+=1;result.total+=1;}
      });
    });
    return result;
  }
  ...
  function recommendations(report){
    ...
    if(report.duplicates.total){list.push("Hay "+report.duplicates.total+" duplicado(s) lógicos. Use Corregir identidades en Base Local para preparar una consolidación con respaldo.");}
    ...
    if(report.orphans.total){list.push("Hay "+report.orphans.total+" relación(es) huérfana(s). Revise la identificación y el período antes de intervenirlas.");}
    if(report.legacy.missingTotal){list.push("Existen "+report.legacy.missingTotal+" registro(s) legacy sin equivalente V2. Ejecute la migración segura.");}
    ...
    list.push("Clave local oficial: cedula__periodoId. Firebase académico conserva periodoId__cedula.");
    list.push("Las tablas legacy permanecen intactas. No se ofrece borrado masivo.");
    return list;
  }

  function analyze(){
    var selected=currentPeriod();
    ...
    return Promise.all([
      Promise.all(unique.map(function(name){return read(name).then(function(rows){return {name:name,rows:rows};});})),
      inspectIndexes()
      ...
      var matriculas=selected.id?allMatriculas.filter(function(row){return rowPeriod(row)===selected.id;}):allMatriculas;
      var requisitos=selected.id?(tables.requisitos_estudiante||[]).filter(function(row){return rowPeriod(row)===selected.id;}):tables.requisitos_estudiante||[];
      var notas=selected.id?(tables.notas_titulacion||[]).filter(function(row){return rowPeriod(row)===selected.id;}):tables.notas_titulacion||[];
      var contactos=selected.id?(tables.contactos_estudiante||[]).filter(function(row){return rowPeriod(row)===selected.id;}):tables.contactos_estudiante||[];
      var divisiones=selected.id?(tables.divisiones_estudiante||[]).filter(function(row){return rowPeriod(row)===selected.id;}):tables.divisiones_estudiante||[];
      var cambios=selected.id?(tables.cambios_pendientes||[]).filter(function(row){return !rowPeriod(row)||rowPeriod(row)===selected.id;}):tables.cambios_pendientes||[];
      var errores=selected.id?(tables.errores_validacion||[]).filter(function(row){return !rowPeriod(row)||rowPeriod(row)===selected.id;}):tables.errores_validacion||[];
      ...
      var duplicates={
        matriculas:duplicateGroups(matriculas,rowStudentId),
        requisitos:duplicateGroups(requisitos,requirementRelationKey),
      ...
      var legacy={
        ...
        requisitos:coverage(tables.requisitos||[],tables.requisitos_estudiante||[],requirementRelationKey),
      ...
      var report={
        ...
        firebaseAcademicId:"periodoId__cedula",
        period:selected,
  ...
  function paint(report){
    ...
    var rows=[
      ...
      ["Clave local",report.canonicalLocalId],
  ...
  function previewMigration(){
    ...
    return migration.preview().then(function(result){
      migrationReady=!!(result&&result.ok);
      var button=id("maintenance-migrate");
      if(button){button.disabled=!migrationReady;}
      var output=id("maintenance-json");
      if(output){output.textContent=JSON.stringify({maintenance:lastReport,migrationPreview:result},null,2);}
      reportStatus("Vista previa lista. No se escribió ningún dato.","success");
      return result;
    });
  }
  ...
  function runMigration(){
    ...
    return safety.then(function(saved){
      ...
      return migration.run({confirm:true}).then(function(result){
        result.safetyBackupId=saved&&saved.record&&saved.record.id;
        migrationReady=false;
        reportStatus(result.message||"Migración finalizada.",result.ok===false?"error":"success");
        return runAndPaint().then(function(){return result;});
      });
    });
  }
  ...
  function resetBlocked(){
    var queue=outbox();
    var selected=currentPeriod();
    if(!queue||typeof queue.list!=="function"||typeof queue.resetRetries!=="function"){return Promise.reject(new Error("La cola de sincronización no permite rehabilitar reintentos."));}
    return queue.list({periodoId:selected.id}).then(function(changes){
      var plan={};
      ["google","firebase","supabase"].forEach(function(target){plan[target]=(changes||[]).filter(function(row){return typeof queue.isBlocked==="function"&&queue.isBlocked(row,target,{});});});
      ...
      Object.keys(plan).forEach(function(target){if(plan[target].length){chain=chain.then(function(){return queue.resetRetries(plan[target],target);});}});

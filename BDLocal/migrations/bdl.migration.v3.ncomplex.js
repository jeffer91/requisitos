...
(function(window){
  ...
  function prepareBaseStores(){
    var api=window.BL2DB;
    if(!api||typeof api.open!=="function"){
      return Promise.reject(new Error(
        "BL2DB debe cargarse antes de ejecutar la migración de Ncomplex."
      ));
    }

    return api.open({force:true,skipRepair:true}).then(function(openedDb){
      try{
    ...
    return new Promise(function(resolve,reject){
      ...
      request.onsuccess=function(){
        ...
        var result={
          ...
          stores:Array.prototype.slice.call(db.objectStoreNames||[])
        };
        db.close();
        resolve(result);
      };
      ...
      request.onerror=function(){
        reject(request.error||new Error("No se pudo inspeccionar Base Local."));
      };

      request.onblocked=function(){
        reject(new Error("Base Local está bloqueada por otra ventana."));
      };
    });
  }
  ...
  function upgrade(version){
    return new Promise(function(resolve,reject){
      ...
      request.onsuccess=function(){
        ...
        var result={
          ok:hasStore(db),
          version:Number(db.version||version),
          store:STORE,
          stores:Array.prototype.slice.call(db.objectStoreNames||[])
        };
        db.close();
      ...
      request.onerror=function(){
        reject(request.error||new Error("No se pudo crear evaluaciones_titulacion."));
      };

      request.onblocked=function(){
        reject(new Error("Cierre otras ventanas para actualizar Base Local."));
      };
    });
  }
  ...
  function reopenBL2(){
    ...
    return api.open({force:true,skipRepair:true}).catch(function(){
      return null;
    });
  }
  ...
  function status(){
    var meta=window.BL2DB&&typeof window.BL2DB.meta==="function"
      ? window.BL2DB.meta()
      : null;

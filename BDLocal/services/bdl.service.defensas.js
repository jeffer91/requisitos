...
(function(window){
  ...
  function copyNotaFields(
    ...
    row.observacionDefensa = text(
      nota.observacion ||
      nota.observacionDefensa ||
      row.observacionDefensa ||
      ""
    );

    row.fechaDefensa = text(
      nota.fechaDefensa ||
      row.fechaDefensa ||
      ""
    );
    ...
    return Promise.all(
      rows.map(function(row){
        ...
        if (
          !periodoId ||
          !cedula ||
          typeof repo.getByPeriodoCedula !==
            "function"
        ){
  ...
  function hydrateWithNotas(
    ...
    var smallLimit =
      Number(
        options.directHydrateLimit ||
        80
      );

    if (
      rows.length <= smallLimit &&
      typeof repo.getByPeriodoCedula ===
        "function"
    ){
      return hydrateByDirectCalls(
        rows,
        options,
        repo
      );
  ...
  function filterDefensas(
    ...
    var estado = text(
      options.estadoDefensa ||
      options.defensaEstado ||
      options.statusDefensa ||
      ""
    );
  ...
  function requiresFullHydration(
    options
  ){
    options = options || {};
  ...
  function getFiltered(options){
    ...
    return service
      ...
      .then(function(rows){
        return hydrateWithNotas(
          rows || [],
          options
        );
      })
      .then(function(rows){
        return filterDefensas(
          rows,
          options
        );
      });
      ...
      rows.forEach(
        function(row){
          ...
          if (
            ...
            stats.articuloNoAprobado +=
              1;
          }else if (
            estado ===
            "PENDIENTE_DEFENSA"
          ){
            stats.pendienteDefensa +=
              1;
          }else if (
            estado ===
            "PENDIENTE_FINAL"
          ){
            stats.pendienteFinal +=
              1;
          }else if (
            estado ===
            "APROBADO"
  ...
  function saveNota(row){
    ...
    if (
      ...
      return Promise.reject(
        new Error(
          "Repositorio de notas no disponible."
        )
      );
    }

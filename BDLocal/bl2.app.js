...
(function(window,document){
  ...
  var VERSION = "2.3.0-nonblocking-startup";
  ...
  function renderKPIs(){
    ...
    return timeout(function(){ return currentCore.getSummary(active.id); },REFRESH_TIMEOUT_MS,"El resumen del período")
      .then(function(summary){
        ...
        if(summary.pendientesGoogle != null){ setText("bl2-kpi-google",formatNumber(summary.pendientesGoogle)); }
        if(summary.pendientesFirebase != null){ setText("bl2-kpi-firebase",formatNumber(summary.pendientesFirebase)); }
        return summary;
      })
      .catch(function(error){
  ...
  function refresh(options){
    ...
    state.refreshPromise = timeout(function(){
      ...
      if(window.BL2CloudPullSafe && typeof window.BL2CloudPullSafe.bind === "function"){
        try{ window.BL2CloudPullSafe.bind(); }catch(error){ log("No se pudo enlazar descarga segura: " + error.message,"warn"); }
      }
  ...
  function setupEvents(){
    ...
    window.addEventListener("bl2:student-updated",function(){ scheduleRefresh("student-updated",220); });
    window.addEventListener("bdlocal:sync-v2-finished",function(){ scheduleRefresh("sync-finished",300); });
  }

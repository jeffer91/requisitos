...
(function(window){
  ...
  function installActiveFilterInline(){
    ...
    function deleted(row){row=row||{};return row.eliminado===true||row._firebaseDeleted===true||text(row.estadoRegistro).toUpperCase()==="ELIMINADO";}
    ...
    window.BDLocalActiveFilter=window.BDLocalActiveFilter||{version:"inline-"+VERSION,sanitize:sanitize,isDeleted:deleted,install:function(){return true;}};
    return true;
  }
  ...
  function loadSharedScript(relative,globalName){
    ...
    var existing=Array.prototype.slice.call(document.scripts||[]).find(function(script){return script.src===url||script.getAttribute("data-bdl-shared-src")===url;});
    if(existing){
  ...
  function loadSharedArchitecture(){
    ...
    var files=[
      ...
      ["../firebase/bdl.firebase.migration-ui.js","RequisitosFirebaseMigrationUI"]
    ];
    var chain=Promise.resolve();
    files.forEach(function(item){chain=chain.then(function(){return loadSharedScript(item[0],item[1]);});});
    return chain.then(function(){
      if(window.BDLFirebaseOutboxPolicy&&typeof window.BDLFirebaseOutboxPolicy.install==="function"){return window.BDLFirebaseOutboxPolicy.install();}
      return null;
    }).then(function(){
      var detail={
  ...
  function cfgStores(){var cfg=window.BL2Config||{};var stores=cfg.stores||{};return {legacy:text(stores.cambios||"cambios"),outbox:text(stores.cambiosPendientes||"cambios_pendientes")};}
  function repository(){
    if(window.BDLRepoCambios){return window.BDLRepoCambios;}
    if(window.BDLRepositories&&typeof window.BDLRepositories.get==="function"){return window.BDLRepositories.get("cambios_pendientes")||window.BDLRepositories.get("cambios");}
    return null;
  }
  function mirrorOne(row,mode){
    ...
    return current.save(row||{},{source:"outbox_bridge",mode:mode||"put"}).then(function(saved){
      if(saved){try{window.dispatchEvent(new CustomEvent("bdlocal:outbox-bridged",{detail:{id:saved.id,logicalKey:saved.logicalKey||"",changedAt:saved.lastContentChangedAt||saved.updatedAt||nowISO(),store:cfgStores().outbox,at:nowISO()}}));}catch(error){}}
      return saved;
  ...
  function mirrorMany(rows,mode){
    var current=repository();rows=Array.isArray(rows)?rows:[];
    if(!rows.length){return Promise.resolve([]);}
    if(current&&typeof current.saveMany==="function"){return current.saveMany(rows,{source:"outbox_bridge",mode:mode||"bulkPut"}).catch(function(){return [];});}
  ...
  function install(){
    installActiveFilterInline();
    var currentDb=window.BL2DB||null;
    if(!currentDb||typeof currentDb.put!=="function"||typeof currentDb.bulkPut!=="function"){return false;}
    if(currentDb.__outboxBridgeInstalled){return true;}
    var originalPut=currentDb.put.bind(currentDb);var originalBulkPut=currentDb.bulkPut.bind(currentDb);
    currentDb.put=function(storeName,value){
      var stores=cfgStores();if(text(storeName)!==stores.legacy){return originalPut(storeName,value);}
      return originalPut(storeName,value).then(function(saved){return mirrorOne(saved||value,"put").then(function(){return saved;});});
    };
    currentDb.bulkPut=function(storeName,rows){
      ...
      return originalBulkPut(storeName,rows).then(function(saved){var sourceRows=Array.isArray(saved)&&saved.length?saved:rows;return mirrorMany(sourceRows,"bulkPut").then(function(){return saved;});});
    };
    currentDb.__outboxBridgeInstalled=true;currentDb.outboxBridgeVersion=VERSION;
  ...
  window.BDLOutboxBridge={version:VERSION,install:install,mirrorOne:mirrorOne,mirrorMany:mirrorMany,installActiveFilterInline:installActiveFilterInline,loadSharedArchitecture:loadSharedArchitecture,ready:function(){return window.BDLSharedArchitectureReady;}};
  installActiveFilterInline();
  window.BDLSharedArchitectureReady=loadSharedArchitecture();
  window.BDLSharedArchitectureReady.catch(function(error){try{console.warn("[BDLOutboxBridge] Arquitectura compartida incompleta",error);}catch(innerError){}});
  install();
})(window);

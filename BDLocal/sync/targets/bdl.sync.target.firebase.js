...
- Usar la referencia remota capturada durante el análisis para controlar conflictos.
...
var VERSION="2.2.0-analyzed-baseline";
...
function repo(name,fallback){
  ...
  return registry&&typeof registry.get==="function"
    ?(registry.get(name)||fallback||null)
    :(fallback||null);
}
...
function requiresStudentIdentity(entity){
  return ["matriculas","requisitos","notas"].indexOf(entity)>=0;
}
...
function localContext(change,options){
  ...
  var requested=entitiesFor(change);
  var needsIdentity=requested.some(requiresStudentIdentity);

  if(needsIdentity&&!current.ok){
    return Promise.resolve({
      ok:false,change:change,payload:payload,entities:requested,
      reason:"Falta período o identificación."
    });
  }
  ...
  var person=current.cedula&&personaRepo&&typeof personaRepo.getByCedula==="function"
    ?personaRepo.getByCedula(current.cedula)
    :Promise.resolve(null);
  var enrollment=current.ok&&matriculaRepo&&typeof matriculaRepo.getByPeriodoCedula==="function"
    ?matriculaRepo.getByPeriodoCedula(current.periodoId,current.cedula)
    :Promise.resolve(null);
  var requirements=current.ok&&requisitosRepo&&typeof requisitosRepo.list==="function"
    ?requisitosRepo.list({periodoId:current.periodoId,cedula:current.cedula})
    :Promise.resolve([]);
  var notes=current.ok&&notasRepo&&typeof notasRepo.getByPeriodoCedula==="function"
    ?notasRepo.getByPeriodoCedula(current.periodoId,current.cedula)
    :Promise.resolve(null);
  ...
  return Promise.all([person,enrollment,requirements,notes]).then(function(values){
    ...
    if(current.cedula){
      base.cedula=current.cedula;
      base.numeroIdentificacion=current.cedula;
    }
    if(current.periodoId){
      base.periodoId=current.periodoId;
      base.periodId=current.periodoId;
    }
    return {
      ok:true,identity:current,payload:payload,change:change,entities:requested,
      base:base,person:values[0],enrollment:values[1],
      requirements:values[2]||[],notes:values[3]
    };
  });
}
...
function withMeta(entity,data,source){
  ...
  var current=mapper();
  var stamp=now();
  data.createdAt=text(data.createdAt)||stamp;
  data.updatedAt=text(data.updatedAt)||stamp;
  data.version=Math.max(1,Number(data.version||1));
  data.eliminado=data.eliminado===true;
  data.eliminadoEn=data.eliminado?(text(data.eliminadoEn)||data.updatedAt):"";
...
function directDocument(entity,context){
  ...
  var changeId=rowId(context.change);
  var current=context.identity||{};

  if(entity==="periodos"){
    data.periodoId=text(data.periodoId||data.id||current.periodoId);
    ...
    data.entidadId=text(data.entidadId||data.registroId||current.localId||data.id);
    data.accion=text(data.accion||data.action||actionOf(context.change));
    ...
    data.id=text(data.id)||
      (text(data.archivoHash)&&data.periodoId
        ?text(data.archivoHash)+"__"+data.periodoId
        :changeId||stableId("importacion",JSON.stringify(data)));
    data.createdAt=text(data.createdAt)||now();
  }

  if(actionOf(context.change)==="DELETE"||data.eliminado===true){
    data.eliminado=true;
    data.eliminadoEn=text(data.eliminadoEn)||now();
  }
...
function documentFor(entity,context){
  ...
  if(entity==="estudiantes"&&typeof current.studentDocument==="function"){
    document=current.studentDocument(context.base);
  }else if(entity==="matriculas"&&typeof current.enrollmentDocument==="function"){
    document=current.enrollmentDocument(context.base);
  }else if(entity==="requisitos"&&typeof current.requirementsDocument==="function"){
    document=current.requirementsDocument(context.base,context.requirements);
  }else if(entity==="notas"&&typeof current.notesDocument==="function"){
    document=current.notesDocument(context.base,context.notes||{});
  }else{
    document=directDocument(entity,context);
  }

  if(document&&(actionOf(context.change)==="DELETE"||context.payload.eliminado===true)){
    document.eliminado=true;
    document.eliminadoEn=text(context.payload.eliminadoEn)||now();
    document=withMeta(entity,document,"BDLocal");
  }
  return document;
}
...
function expectedFor(base){
  base=base||{};
  var known=!!(
    text(base._firebaseDataHash)||
    Number(base._firebaseVersion)>0||
    text(base._firebaseUpdatedAt)
  );
  return known?{
    exists:true,
...
function expectedOverride(options,entity,documentId){
  var map=options&&options.expectedByDocument||{};
  var key=expectedKey(entity,documentId);
  return Object.prototype.hasOwnProperty.call(map,key)?clone(map[key]):null;
}
...
function prepareEntries(rows,options){
  ...
  return Promise.all(safeRows(rows,options).map(function(change){
    return localContext(change,options).then(function(context){
      if(!context.ok){
        skipped.push({id:rowId(change),reason:context.reason});
        return [];
      }
      ...
      context.entities.forEach(function(entity){
        ...
        var documentId=central&&typeof central.documentId==="function"
          ?central.documentId(entity,document||{})
          :text(document&&document.id);

        if(!document||(!documentId&&["historial","importaciones"].indexOf(entity)<0)){
          skipped.push({
            id:rowId(change),entity:entity,
            reason:"No se pudo formar el documento completo."
          });
          return;
        }
        ...
        entries.push({
          ...
          document:document,
          expected:expectedOverride(options,entity,documentId)||expectedFor(baseFor(entity,context)),
          changeIds:[rowId(change)].filter(Boolean),
          context:context
        });
      });
      return entries;
    ...
    groups.forEach(function(list){
      (list||[]).forEach(function(item){
        ...
        item.changeIds.forEach(function(id){
          if(map[key].changeIds.indexOf(id)<0){map[key].changeIds.push(id);}
        });
      });
    });

    return {
      entries:Object.keys(map).map(function(key){return map[key];}),
      skipped:skipped
    };
  });
}
...
function quotaFor(writes){
  ...
  if(!current||typeof current.getFirebaseQuotaStatus!=="function"){
    return {
      allowed:true,level:"sin_control",
      estimatedOps:Number(writes||0),
      message:"Control de cuota no disponible."
    };
  }
...
function registerUsage(writes){
  try{
    ...
    if(current&&typeof current.registerFirebaseUsage==="function"){
      current.registerFirebaseUsage({
        writes:Number(writes||0),
        label:"Firebase V2: escrituras confirmadas."
      });
    }
  }catch(error){}
}
...
function markStateStart(entities,periodoId){
  ...
  if(!current||typeof current.begin!=="function"){
    return Promise.reject(new Error("Repositorio sync_estado no disponible."));
  }
  return Promise.all(entities.map(function(entity){
    return current.begin("firebase",entity,periodoId,"push");
  }));
}

function markStateSuccess(counts,conflictCounts,periodoId){
  var current=syncState();
  if(!current||typeof current.pushSuccess!=="function"){
    return Promise.reject(new Error("Repositorio sync_estado no disponible."));
  }
  var entities=Object.keys(Object.assign({},counts,conflictCounts));
  return Promise.all(entities.map(function(entity){
    return current.pushSuccess(entity,periodoId,{
      writeCount:counts[entity]||0,
      conflictCount:conflictCounts[entity]||0,
      batchCount:1,
      at:now()
    });
  }));
...
function markStateError(entities,periodoId,error){
  var current=syncState();
  if(!current||typeof current.fail!=="function"){
    return Promise.resolve([]);
  }
  return Promise.all(entities.map(function(entity){
    return current.fail("firebase",entity,periodoId,error,{mode:"push"});
  }));
}

function saveConflict(entry,conflict){
  var current=conflictRepo();
  ...
  return current.save({
    ...
    cedula:id.cedula,
    motivo:"ATOMIC_REMOTE_CONFLICT",
    local:entry.document,
    remote:conflict&&conflict.remote,
...
function writeEntries(entries){
  var central=repository();
  if(!central||typeof central.writeManyChecked!=="function"){
    return Promise.reject(new Error("Repositorio Firebase V2 seguro no disponible."));
  }

  var grouped=Object.create(null);
  entries.forEach(function(entry){
    (grouped[entry.entity]=grouped[entry.entity]||[]).push(entry);
  });

  var results=[];
  var counts={};
  var conflictCounts={};
  var successKeys=Object.create(null);
  var conflictKeys=Object.create(null);
  var chain=Promise.resolve();

  Object.keys(grouped).forEach(function(entity){
    chain=chain.then(function(){
      var source=grouped[entity];
      return central.writeManyChecked(
        entity,
        ...
        {merge:false,allowUnbasedOverwrite:false}
      ).then(function(result){
        results.push(result);
        counts[entity]=Number(result.written||0);
        conflictCounts[entity]=Number(result.conflicts&&result.conflicts.length||0);

        (result.results||[]).forEach(function(item){
          successKeys[entity+"__"+item.documentId]=true;
        });

        return Promise.all((result.conflicts||[]).map(function(conflict){
          var key=entity+"__"+text(conflict.documentId);
          conflictKeys[key]=true;
          var entry=source.filter(function(item){
            return item.documentId===conflict.documentId;
          })[0];
          if(!entry){
            return Promise.reject(new Error("No se pudo relacionar el conflicto con la cola local."));
          }
          return saveConflict(entry,conflict);
        }));
      });
    });
  });
  ...
  return chain.then(function(){
    return {
      ...
      written:Object.keys(counts).reduce(function(sum,key){
        return sum+counts[key];
      },0),
      conflicts:Object.keys(conflictKeys).length
    };
  });
}
...
function processedChangeIds(entries,written){
  ...
  entries.forEach(function(entry){
    var key=entry.entity+"__"+entry.documentId;
    var success=written.successKeys[key]===true;
    entry.changeIds.forEach(function(id){
      if(!status[id]){status[id]={total:0,success:0};}
      status[id].total+=1;
      if(success){status[id].success+=1;}
    });
  });
  return Object.keys(status).filter(function(id){
    return status[id].total>0&&status[id].success===status[id].total;
  });
}
...
function push(pendingRows,options){
  ...
  if(options.manual!==true){
    return Promise.resolve({
      ok:false,target:"firebase",blocked:true,deferWithoutAttempt:true,
      processedIds:[],message:"Firebase V2 solo admite sincronización manual."
    });
  }
  if(!repository()||!mapper()||!identity()){
    return Promise.resolve({
      ok:false,target:"firebase",blocked:true,deferWithoutAttempt:true,
      processedIds:[],message:"La arquitectura Firebase V2 todavía no está cargada."
    });
  }
  if(!conflictRepo()||!syncState()){
    return Promise.resolve({
      ok:false,target:"firebase",blocked:true,deferWithoutAttempt:true,
      processedIds:[],message:"Los repositorios de estado y conflictos todavía no están disponibles."
    });
  }
  ...
  return prepareEntries(pendingRows,options).then(function(result){
    ...
    if(!result.entries.length){
      return {
        stop:true,
        result:{
          ok:false,target:"firebase",processedIds:[],
          skipped:result.skipped,
          message:"Ningún cambio produjo documentos válidos."
        }
      };
    }
    ...
    if(quota.allowed===false){
      return {
        stop:true,
        result:{
          ok:false,target:"firebase",blocked:true,quotaBlocked:true,
          deferWithoutAttempt:true,processedIds:[],
          skipped:result.skipped,quota:quota,
          message:quota.message||"Firebase bloqueado por cuota."
        }
      };
    }

    return markStateStart(entities,text(options.periodoId)).then(function(){
      return {stop:false};
    });
  }).then(function(stage){
    ...
    return writeEntries(prepared.entries).then(function(written){
      var processed=processedChangeIds(prepared.entries,written);
      var allConflicted=written.conflicts>0&&processed.length===0;
      registerUsage(written.written);

      return markStateSuccess(
        written.counts,
        written.conflictCounts,
        text(options.periodoId)
      ).then(function(){
        return {
          ok:!allConflicted,
          blocked:allConflicted,
          deferWithoutAttempt:allConflicted,
          partial:written.conflicts>0,
          target:"firebase",
          schema:"V2",
          collections:entities,
          documentsPrepared:prepared.entries.length,
          documentsWritten:written.written,
          conflicts:written.conflicts,
          processedIds:processed,
          skipped:prepared.skipped,
          quota:quota,
          results:written.results,
          message:written.conflicts
            ?"Se procesó el lote parcialmente. Los cambios con conflicto permanecen pendientes."
            :"Firebase V2 actualizado sin conflictos."
        };
      });
    });
  }).catch(function(error){
    return markStateError(entities,text(options.periodoId),error).then(function(){
      return {
        ok:false,target:"firebase",processedIds:[],

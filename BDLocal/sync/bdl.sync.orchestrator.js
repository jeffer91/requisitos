...
Función:
- Orquestar sincronización desde cambios_pendientes.
...
(function(window){
  ...
  function callLegacyManager(target, options){
    ...
    if(!options.allowLegacyManager){
      return Promise.resolve({
        ...
        safe:true,
        target:target,
        message:"No se marcó la cola como sincronizada: falta adaptador real para cambios_pendientes en " + target + "."
      });
    }
    ...
    return Promise.resolve({ ok:false, skipped:true, safe:true, message:"Destino sin implementación: " + target });
  }
  ...
  function syncTarget(target, options){
    ...
    return ob.pending(target, options).then(function(pendingRows){
      if(!pendingRows.length){ return { ok:true, target:target, pending:0, message:"No hay pendientes para " + target + "." }; }
      ...
      return callRegisteredTarget(target, pendingRows, options).then(function(result){
        ...
        if(!confirmedRows.length){
          return {
            ...
            pending:pendingRows.length,
            marked:0,
            message:"El destino respondió, pero no confirmó procesamiento de cambios_pendientes. No se marcó como SINCRONIZADO."
          };
        }
  ...
  function syncQueue(options){
    options = options || {};
    if(state.running){ return Promise.resolve({ ok:false, running:true, message:"Ya existe una sincronización en curso." }); }
    state.running = true;
    ...
    enabledTargets.forEach(function(target){
      chain = chain.then(function(){
        return syncTarget(target, options).then(function(result){ results.push(result); return result; });
      });
    });
    ...
    return chain.then(function(){
      ...
      state.running = false;
      state.lastResult = { ok:failed.length === 0, safe:skipped.length > 0, results:results, finishedAt:new Date().toISOString() };
  ...
  function status(){
    var ob = outbox();
    var base = { version:VERSION, running:state.running, lastRunAt:state.lastRunAt, lastResult:state.lastResult, lastError:state.lastError, safeMode:true };
  ...
  window.BDLSyncOrchestrator = { version:VERSION, syncTarget:syncTarget, syncQueue:syncQueue, status:status };
})(window);

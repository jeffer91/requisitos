...
- Evitar que una apertura pendiente congele Base Local indefinidamente.
========================================================= */
(function(window){
  "use strict";
  ...
  function requestToPromise(request){
    return new Promise(function(resolve, rejectFn){
      ...
      request.onerror = function(){
        rejectFn(request.error || new Error("Error IndexedDB."));
      };
    });
  }
  ...
  function requiredStoreNames(){
    var fromConfig =
      config.dbV2 &&
      Array.isArray(config.dbV2.requiredStores)
        ? config.dbV2.requiredStores
        : [];
  ...
  function missingRequiredStores(db){
    ...
    objectStoreNames(db).forEach(function(name){
      existing[name] = true;
    });
  ...
  function openWithVersion(version){
    ...
    return new Promise(function(resolve, rejectFn){
      ...
      var timeoutId = window.setTimeout(function(){
        ...
        rejectFn(new Error(
          "La apertura de Base Local excedió 12 segundos. " +
          "Cierre completamente otras ventanas de la aplicación " +
      ...
      request.onupgradeneeded = function(event){
        try{
          ...
          ensureStores(upgradeDb, event.target.transaction);
        }catch(error){
          try{
      ...
      request.onsuccess = function(event){
        ...
        openedDb.onversionchange = function(){
          ...
          if(state.db === openedDb){
            state.db = null;
          }
        };

        resolveOnce(openedDb);
      };

      request.onerror = function(){
        rejectOnce(
          request.error ||
          new Error("No se pudo abrir Base Local.")
        );
      };

      request.onblocked = function(){
        rejectOnce(new Error(
          "Base Local está bloqueada por otra pestaña o ventana. " +
          "Cierre completamente las otras ventanas de la aplicación " +
          "y vuelva a abrir."
        ));
      };
    });
  ...
  function open(options){
    ...
    state.opening = firstOpen
      .then(function(openedDb){
        var missing = missingRequiredStores(openedDb);

        if(missing.length && !options.skipRepair){
          var nextVersion = Math.max(
            Number(openedDb.version || state.version) + 1,
            state.version + 1
          );
        ...
        state.version = Math.max(
          Number(openedDb.version || state.version),
          state.version
        );
  ...
  function tx(storeNames, mode){
    return open().then(function(openedDb){
      var list = Array.isArray(storeNames)
        ? storeNames
        : [storeNames];
      ...
      list.forEach(function(name){
        if(!openedDb.objectStoreNames.contains(name)){
          throw new Error(
            ...
            ". Ejecute diagnóstico/migración de BDLocal."
          );
        }
      });

      return openedDb.transaction(list, mode || "readonly");
    });
  }
  ...
  function store(name, mode){
    ...
    if(!name){
      return reject("Tabla no especificada.");
    }
    ...
    return store(storeNameValue, "readonly").then(function(objectStore){
      ...
      return new Promise(function(resolve, rejectFn){
        ...
        request.onerror = function(){
          rejectFn(
            request.error ||
            new Error("Error leyendo " + storeNameValue)
          );
        };
      });
    });
  ...
  function bulkPut(storeNameValue, rows){
    ...
    return open().then(function(openedDb){
      return new Promise(function(resolve, rejectFn){
        if(!openedDb.objectStoreNames.contains(storeNameValue)){
          rejectFn(new Error(
            "No existe la tabla " + storeNameValue + "."
          ));
          return;
        }

        var transaction = openedDb.transaction(
          [storeNameValue],
          "readwrite"
        );
        ...
        transaction.onerror = function(){
          rejectFn(
            transaction.error ||
            new Error(
              "No se pudo guardar lote en " + storeNameValue
            )
          );
        };
        ...
        transaction.onabort = function(){
          rejectFn(
            transaction.error ||
            new Error(
              "Se canceló el guardado en " + storeNameValue
            )
          );
        };
  ...
  function queryByIndex(storeNameValue, indexName, value){
    ...
    if(!indexName){
      return reject("Índice no especificado.");
    }
    ...
    return store(storeNameValue, "readonly").then(function(objectStore){
      return new Promise(function(resolve, rejectFn){
        ...
        request.onerror = function(){
          rejectFn(
            request.error ||
            new Error("Error consultando índice.")
          );
        };
      });
    });
    ...
    return store(storeNameValue, "readonly").then(function(objectStore){
      return new Promise(function(resolve, rejectFn){
        ...
        request.onerror = function(){
          rejectFn(
            request.error ||
            new Error("Error consultando rango.")
          );
        };
      });
    });
  ...
  function setSetting(key, value){
    ...
    if(!key){
      return reject("Clave de configuración vacía.");
    }
  ...
  function importAll(payload, options){
    ...
    names.forEach(function(name){
      chain = chain.then(function(){
        var rows = Array.isArray(tables[name])
          ? tables[name]
          : [];
  ...
  function resetAll(){
    ...
    names.forEach(function(name){
      chain = chain.then(function(){
        return clear(name).catch(function(){
          return true;
        });
      });
    });
  ...
  function deleteDatabase(){
    ...
    return new Promise(function(resolve, rejectFn){
      ...
      request.onerror = function(){
        rejectFn(
          request.error ||
          new Error("No se pudo eliminar BL2.")
        );
      };

      request.onblocked = function(){
        rejectFn(new Error(
          "No se pudo eliminar BL2 porque todavía está abierta."
        ));
      };
    });
  }
  ...
  function meta(){
    var openedDb = state.db || null;
    var names = openedDb
      ? objectStoreNames(openedDb)
      : allStoreNames();
    ...
    return {
      ...
      missingStores: openedDb
        ? missingRequiredStores(openedDb)
        : [],
      schemaVersion:
  ...
  window.BL2DB = {
    version: "2.1.1-safe-indexeddb-timeout",

...
(function(window,document){
  ...
  var VERSION="1.1.0-reliable-change-events";
  ...
  function screenName(){
    ...
    var value=text(body&&(body.getAttribute("data-bdl-screen")||body.getAttribute("data-screen"))).toLowerCase();
    if(value){return value;}
    try{
      if(window.BDLocalConeRegistry&&typeof window.BDLocalConeRegistry.detect==="function"){
        value=text(window.BDLocalConeRegistry.detect("")).toLowerCase();
      }
    }catch(error){}
    if(value){return value;}
    return text(window.location&&window.location.pathname).toLowerCase().replace(/\\/g,"/");
  }
  function isGlobalScreen(){
    var current=screenName();
    return current==="global"||current.indexOf("/global/")>=0||current.indexOf("global.html")>=0;
  }
  ...
  function writeStored(period){
    try{
      ...
      window.localStorage.setItem(SIGNAL_KEY,JSON.stringify({id:period.id,updatedAt:period.updatedAt,nonce:Math.random().toString(16).slice(2)}));
      return true;
    }catch(error){return false;}
  }
  function selectorIdentity(select){
    return text(select&&(select.id||select.name||select.getAttribute("data-periodo-global")||select.getAttribute("aria-label")));
  }
  function isEligibleSelect(select){
    if(!state.enabled||!select||text(select.tagName).toUpperCase()!=="SELECT"){return false;}
    if(select.getAttribute("data-periodo-global")==="false"){return false;}
    if(select.getAttribute("data-periodo-global")==="true"){return true;}
    var identity=[select.id,select.name,select.getAttribute("data-filter"),select.getAttribute("data-global-filter"),select.getAttribute("aria-label")].map(text).join(" ").toLowerCase();
    if(!/(periodo|período|period)/.test(identity)){return false;}
    if(/(desde|hasta|inicio|fin|rango|range|global)/.test(identity)){return false;}
    return true;
  }
  function optionFor(select,id){
    ...
    for(var index=0;index<options.length;index+=1){
      if(canonicalPeriodId(options[index].value)===id){return options[index];}
    }
    return null;
  }
  ...
  function createChangeEvent(period){
    try{
      return new CustomEvent("change",{bubbles:true,detail:{periodoGlobal:true,periodoId:period.id,periodoLabel:period.label}});
    }catch(error){
      try{
        var event=document.createEvent("Event");
        event.initEvent("change",true,false);
        event.periodoGlobal=true;
        event.periodoId=period.id;
        event.periodoLabel=period.label;
        return event;
      }catch(innerError){return null;}
    }
  }
  function isProgrammaticEvent(event){
    return !!(event&&((event.detail&&event.detail.periodoGlobal===true)||event.periodoGlobal===true));
  }
  function emit(period,source,external){
    ...
    try{window.dispatchEvent(new CustomEvent(EVENT_CHANGED,{detail:detail}));}catch(error){}
    return detail;
  }
  function broadcast(period,source){
    if(!state.channel){return;}
    try{state.channel.postMessage({type:"period-changed",period:clone(period),source:text(source||"periodo_global"),at:nowISO()});}catch(error){}
  }
  function applyToSelect(select,period,options){
    ...
    if(previous!==period.id&&options.dispatch!==false){
      var event=createChangeEvent(period);
      if(event){try{select.dispatchEvent(event);}catch(error){}}
    }
    return true;
  }
  ...
  function syncCore(period){
    ...
    if(!state.enabled||!period||state.syncingCore||!core||typeof core.setActivePeriod!=="function"){return Promise.resolve(null);}
    var signature=period.id+"|"+period.label;
    if(state.lastCoreSignature===signature){return Promise.resolve(period);}
    ...
    return current.then(function(active){
      ...
      if(activeId===period.id){state.lastCoreSignature=signature;return active;}
      return Promise.resolve(core.setActivePeriod(period.id,period.label)).then(function(result){state.lastCoreSignature=signature;return result||period;});
    }).catch(function(){return null;}).finally(function(){state.syncingCore=false;});
  }
  function setPeriod(value,label,options){
    options=options||{};
    if(!state.enabled&&options.force!==true){return clone(state.period);}
    ...
    if(!changed){
      if(options.apply!==false){applyToAll(previous,{dispatch:options.dispatch!==false});}
  ...
  function scheduleScan(delay){
    try{window.clearTimeout(state.scanTimer);}catch(error){}
  ...
  function loadFromCore(){
    ...
    var core=window.BL2Core||null;
    if(!core||typeof core.getActivePeriod!=="function"){return Promise.resolve(null);}
    return Promise.resolve(core.getActivePeriod()).then(function(period){
      if(!period){return null;}
      return setPeriod(period,period.label||period.periodoLabel,{source:"BL2Core",core:false});
    }).catch(function(){return null;});
  }
  function adoptExternal(value,source){
    var period=normalizePeriod(value,value&&value.label,source||"external");
    if(!period){return null;}
    return setPeriod(period,period.label,{source:source||"external",persist:true,broadcast:false,external:true});
  }
  function bindCommunication(){
    window.addEventListener("storage",function(event){
      if(event.key!==STORAGE_KEY||!event.newValue){return;}
      try{adoptExternal(JSON.parse(event.newValue),"storage");}catch(error){}
    });
    if(typeof window.BroadcastChannel==="function"){
      try{
        ...
        state.channel.onmessage=function(event){
          ...
          if(message.type==="period-changed"&&message.period){adoptExternal(message.period,"broadcast");}
        };
      }catch(error){state.channel=null;}
    }
  }
  function bindLifecycle(){
    ["DOMContentLoaded","load","bl2:core-ready","bl2:ready","bl2:app-refreshed","bdlocal:connections-ready","bdlocal:connections-cache-updated","stats:cache-invalidated"].forEach(function(name){
      window.addEventListener(name,function(){loadFromCore();scheduleScan(name==="DOMContentLoaded"?0:40);});
    });
  ...
  function init(){
    ...
    try{
      window.dispatchEvent(new CustomEvent(EVENT_READY,{detail:{enabled:state.enabled,period:clone(state.period),globalIndependent:!state.enabled,version:VERSION,at:nowISO()}}));
    }catch(error){}
    return api;
  }

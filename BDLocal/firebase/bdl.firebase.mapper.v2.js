...
(function(window){
  ...
  var VERSION="1.1.0-stable-functional-hash";
  ...
  function requirementsDocument(row,requirementRows){
    ...
    if(!current||!current.ok){return null;}
    var values={};var canonicalNames=Object.create(null);
    requirementFields().forEach(function(name){
      canonicalNames[normalizeKey(name)]=name;
      var value=first(row,[name]);if(value!==""){values[name]=value;}
    });
    requirementRows.forEach(function(item){
      if(item&&item.eliminado===true){return;}
      var key=requirementKey(item);var normalized=normalizeKey(key);var value=requirementValue(item);

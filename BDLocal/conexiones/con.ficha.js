/* =========================================================
Nombre completo: con.ficha.js
Ruta o ubicacion: /Requisitos/BDLocal/conexiones/con.ficha.js
Funcion:
- Conectar Ficha con BDLocal.
- Exponer busqueda sincronica desde cache y actualizacion async.
========================================================= */
(function(window){
  "use strict";
    ...
    return HUB.ensureCoreReady().then(function(){
      if(window.BL2Core && typeof window.BL2Core.updateStudent === "function"){
        return window.BL2Core.updateStudent(id, changes, options).then(function(saved){
          return HUB.refreshCache({ source:"con.ficha.updateStudent" }).then(function(){ return saved; });
        });
      }
  ...
  function forFicha(id, options){
    ...
    return {
      found:!!student,
      student:student,
      source:"BDLocalConFicha"
    };
  }

  var api = {
    version:"1.0.0",
    source:"BDLocal/conexiones/con.ficha.js",
    ready:HUB.ready,
    refresh:function(){ return HUB.refreshCache({ source:"con.ficha.refresh" }); },
    periods:periods,
    listPeriods:periods,
    getPeriods:periods,
    rows:rows,
    listStudents:function(options){ return { ok:true, rows:rows(options || {}), total:rows(options || {}).length, source:"BDLocalConFicha" }; },
    getStudents:rows,
    filter:rows,
    divisions:divisions,

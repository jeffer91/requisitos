(function(window){
  ...
  if(!HUB||!U){return;}
  function rows(filters){return U.filterStudents(U.readCache().students,filters||{});}
  function requirements(filters){
    ...
    return (U.readCache().requirements||[]).filter(function(req){
      return (!periodoId||U.samePeriod(req.periodoId,periodoId))&&(!cedula||U.normalizeCedula(req.cedula)===cedula);
    });
  }
  ...
  function buildReportData(filters){
    ...
    return {ok:true,source:"BDLocalConReportes",filters:U.clone(filters),generatedAt:U.nowISO(),estudiantes:estudiantes,rows:estudiantes,requisitos:requisitos,periodos:periods(),resumen:{totalEstudiantes:estudiantes.length,totalRequisitos:requisitos.length}};
  }
  var api={
    version:"1.0.0",
    source:"BDLocal/conexiones/cone.reportes.js",
    ready:HUB.ready,
    refresh:function(){return HUB.refreshCache({source:"cone.reportes.refresh"});},

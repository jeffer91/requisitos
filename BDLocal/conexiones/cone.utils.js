...
- Evitar lecturas síncronas, clonaciones profundas y normalizaciones repetidas.
- Compartir cambios entre ventanas o iframes.
- Conservar la última caché válida si localStorage no admite archivos grandes.
...
(function(window){
  ...
  var MESSAGE = {
    publish:
      "requisitos:bdlocal-cache:publish",

    request:
      "requisitos:bdlocal-cache:request",

    response:
      "requisitos:bdlocal-cache:response",

    updated:
      "requisitos:bdlocal-cache:updated"
  };
  ...
  function normalizePeriod(period){
    ...
    if (
      period.__bdlConUtilsPeriodVersion ===
      VERSION
    ){
      return period;
    }
    ...
    result.__bdlConUtilsPeriodVersion =
      VERSION;
  ...
  function normalizeStudent(row){
    ...
    if (
      row.__bdlConUtilsStudentVersion ===
      VERSION
    ){
      return row;
    }
    ...
    if (
      ...
      result._legacyStudentId =
        result._legacyStudentId ||
        previousId;
    }

    result.id = resolvedId;
    result._id = resolvedId;
    result.studentId = resolvedId;
    result.idEstudiantePeriodo =
      resolvedId;
    ...
    result.__bdlConUtilsStudentVersion =
      VERSION;
  ...
  function normalizeCache(cache){
    ...
    if (
      cache.meta &&
      cache.meta.__bdlConUtilsCacheVersion ===
      VERSION
    ){
      return cache;
    }
    ...
    var result = {
      ...
      students:
        array(cache.students)
          .map(normalizeStudent),

      requirements:
        array(cache.requirements),

      summaries:
        object(cache.summaries),

      diagnostics:
        array(cache.diagnostics)
    };
    ...
    result.meta.totalRequirements =
      result.requirements.length;

    result.meta.periodoId =
      canonicalPeriodId(result.meta.periodoId || "");

    result.meta.tablesRead =
      uniqueStrings(result.meta.tablesRead || []);

    result.meta.tablesChanged =
      uniqueStrings(result.meta.tablesChanged || []);

    result.meta.operation =
      text(result.meta.operation).toLowerCase();

    result.meta.sourceScreen =
      text(result.meta.sourceScreen).toLowerCase();
    ...
    result.meta.__bdlConUtilsCacheVersion =
      VERSION;
  ...
  function writeCache(
    ...
    if (
      options.respectIncomingRevision ===
        true &&
      incomingRevision > 0 &&
      incomingRevision <=
        currentRevision
      ...
      Object.assign(
        ...
        prepared.meta,
        {
          ...
          tablesRead:
            uniqueStrings(
              options.tablesRead ||
              prepared.meta.tablesRead ||
              []
            ),

          tablesChanged:
            uniqueStrings(
              options.tablesChanged ||
              prepared.meta.tablesChanged ||
              []
            ),
          ...
          sourceScreen:
            text(
              ...
              prepared.meta.sourceScreen ||
              "bdlocal"
            ).toLowerCase(),

          __bdlConUtilsCacheVersion:
            VERSION
        }
      );
    ...
    if (
      ...
      emit(
        "bdlocal:conexiones-cache-updated",
        {
          ...
          tablesRead:
            uniqueStrings(prepared.meta.tablesRead || []),

          tablesChanged:
            uniqueStrings(prepared.meta.tablesChanged || []),

          operation:
            prepared.meta.operation || "refresh",

          sourceScreen:
            prepared.meta.sourceScreen || "bdlocal",
    ...
    if (
      ...
      emit(
        "bdlocal:screen-data-updated",
        {
          ...
          periods:
            shared.periods.length,

          students:
            shared.students.length,
  ...
  function filterStudents(
    ...
    for (
      ...
      var row =
        (
          input &&
          input.__bdlConUtilsStudentVersion ===
          VERSION
        )
          ? input
          : normalizeStudent(
    ...
    status:
      function(){
        ...
        return {
          ...
          periodoId:
            text(cache.meta && cache.meta.periodoId),

          tablesRead:
            uniqueStrings(cache.meta && cache.meta.tablesRead || []),

          tablesChanged:
            uniqueStrings(cache.meta && cache.meta.tablesChanged || []),

          operation:
            text(cache.meta && cache.meta.operation),

          sourceScreen:
            text(cache.meta && cache.meta.sourceScreen),

          updatedAt:
            text(cache.meta && cache.meta.updatedAt),

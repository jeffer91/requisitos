...
- Mantener Defensas legacy deshabilitado incluso en diagnósticos.
========================================================= */
(function(window){
  "use strict";

  var VERSION="1.1.0-one-screen-one-connector";
  var definitions=[
  ...
  var tables={
    ...
    tabla:["contactos_estudiante"],ficha:["contactos_estudiante","notas_titulacion","divisiones_estudiante"],
    stats:["notas_titulacion","divisiones_estudiante"],coordi:["contactos_estudiante","divisiones_estudiante"],
  ...
  function definition(item){return {label:item.label,global:item.global,file:item.file,pathHints:item.pathHints.slice(),aliases:[item.id],canRead:true,canWrite:item.canWrite===true,operations:item.canWrite===true?["ready","read","save","update","refresh","status","diagnose"]:["ready","read","refresh","status","diagnose"],tables:unique((tables.common||[]).concat(tables[item.id]||[])),description:"Conector exclusivo de la pantalla "+item.label+"."};}
  function apply(){
    var registry=window.BDLocalConeRegistry;
    if(!registry||typeof registry.register!=="function"){return false;}
    registry.register("defensas",{label:"Defensas legacy",global:"ConDefensas",file:"cone.defensas.js",pathHints:["__legacy_defensas__"],aliases:["defensas_legacy"],canRead:true,canWrite:true,enabled:false,operations:["ready","read","save","update","refresh","status"],tables:unique((tables.common||[]).concat(["notas_titulacion","divisiones_estudiante"])),description:"Compatibilidad interna; ninguna pantalla debe declararse como defensas."});
    definitions.forEach(function(item){registry.register(item.id,definition(item));});
    return true;
  }
  function status(){
    var registry=window.BDLocalConeRegistry;
    var rows=definitions.map(function(item){var registered=registry&&typeof registry.get==="function"?registry.get(item.id):null;return {id:item.id,file:item.file,global:item.global,registered:!!registered,loaded:!!window[item.global]};});
    return {ok:rows.every(function(row){return row.registered;}),version:VERSION,total:rows.length,registered:rows.filter(function(row){return row.registered;}).length,loaded:rows.filter(function(row){return row.loaded;}).length,missing:rows.filter(function(row){return !row.registered;}).map(function(row){return row.id;}),screens:rows};
  }

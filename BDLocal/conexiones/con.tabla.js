/* =========================================================
Nombre completo: con.tabla.js
Ruta o ubicacion: /Requisitos/BDLocal/conexiones/con.tabla.js
Funcion:
- Conectar Tabla con BDLocal usando cache sincrona y BL2 async.
- Exponer aliases listStudents/listPeriods/filterStudents.
========================================================= */
(function(window){
  "use strict";
  ...
  function listAllStudents(){
    return listStudents({ matricula:"" }).rows;
  }
  ...
  function listStudentsByStatus(status, periodoId){
    return listStudents({ matricula:status || "", periodoId:periodoId || "" }).rows;
  }
  ...
  function getStudentById(id, options){
    ...
    options = options || {};
    if(!id){ return null; }
    return listStudents(Object.assign({}, options, { matricula:options.matricula == null ? "" : options.matricula })).rows.filter(function(row){
  ...
  function getStudentByCedula(cedula, periodoId){
    ...
    return listStudents({ periodoId:periodoId || "", matricula:"" }).rows.filter(function(row){
      return U.normalizeCedula(row.cedula || row.numeroIdentificacion) === cedula;
    })[0] || null;
  }

  function refresh(){
    return HUB.refreshCache({ source:"con.tabla.refresh" });
  }

  var api = {
    version:"1.0.0",
    source:"BDLocal/conexiones/con.tabla.js",
    ready:HUB.ready,
    refresh:refresh,

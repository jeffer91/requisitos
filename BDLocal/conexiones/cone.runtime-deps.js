...
- Evitar que un repositorio o servicio se ejecute antes de que exista su API base.
- Reutilizar promesas y scripts ya cargados para Defensas, Ncomplex, Cr-def e InPVC.
========================================================= */
(function(window,document){
  "use strict";

  var VERSION="1.0.0-ordered-runtime";
  ...
  function waitFor(test,label,timeout){
    timeout=Math.max(500,Number(timeout||12000));
    var started=Date.now();
    return new Promise(function(resolve,reject){
      (function check(){
        var value=null;
        try{value=test&&test();}catch(error){}
  ...
  function load(relative,test){
    ...
    if(existing(src)){
      state.reuses+=1;
      return test?waitFor(test,relative,12000):Promise.resolve(src);
    }
    state.loads+=1;
    loading[src]=new Promise(function(resolve,reject){
      var script=document.createElement("script");
      script.src=src;script.async=false;script.defer=false;
      script.setAttribute("data-bdl-runtime-src",src);
  ...
  function sequence(files){
    return (files||[]).reduce(function(chain,item){
      return chain.then(function(){return load(item.path,item.test);});
    },Promise.resolve());
  }
  ...
  function ensureBase(){
    ...
    return sequence(baseFiles()).then(function(){
  ...
  function ensure(profile){
    ...
    return ensureBase().then(function(){return sequence(profiles[profile]);}).then(function(){
      state.profileReady[profile]=true;state.lastError="";return status();

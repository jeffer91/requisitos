...
(function(window,document){
  ...
  function requirementsRepo(){return window.BDLRepoRequisitos||(window.BDLRepositories&&typeof window.BDLRepositories.get==="function"?(window.BDLRepositories.get("requisitos")||window.BDLRepositories.get("requisitos_estudiante")):null);}
  ...
  function canonicalPeriod(value){var utils=window.BDLocalConUtils;return utils&&typeof utils.canonicalPeriodId==="function"?utils.canonicalPeriodId(value):text(value).replace(/_+/g,"__");}
  function samePeriod(a,b){var utils=window.BDLocalConUtils;return utils&&typeof utils.samePeriod==="function"?utils.samePeriod(a,b):canonicalPeriod(a)===canonicalPeriod(b);}
  ...
  function isPVCPeriod(row){
    ...
    var value=norm([periodId(row),periodLabel(row)].join(" "));if(value.indexOf("pvc")>=0){return true;}
  ...
  function ensureDependencies(){
    ...
    return load("../adapters/bdl.screen-deps.js",function(){return window.BDLocalScreenDeps;})
      ...
      .then(function(){
        if(!studentService()||!periodService()||!notesRepo()){throw new Error("Los servicios de períodos, estudiantes o notas no están disponibles para InPVC.");}
  ...
  function listStudents(options){
    options=options||{};var wanted=canonicalPeriod(options.periodoId||options.periodId||"");
    return requireReady().then(function(){return Promise.all([studentService().list({periodoId:wanted,matricula:""}),notesRepo().list({periodoId:wanted}),listRequirements({periodoId:wanted})]);}).then(function(values){

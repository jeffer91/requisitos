...
- Cargar el núcleo BL2 y registrar los conectores de cada pantalla.
...
(function(window,document){
  ...
  var VERSION="1.4.0-refresh-dedup-incremental";
  ...
  var state={
    ...
    lastCompleted:Object.create(null),
    lastRefreshMode:"",
    lastRefreshKey:"",
    metrics:{
      requested:0,
  ...
  function add(file){
    return new Promise(function(resolve){
      ...
      script.onerror=function(){
        state.errors.push({
          file:file,
          message:"No se pudo cargar el script.",
          at:U.nowISO()
        });
  ...
  function ensureCoreScripts(){
    ...
    if(
      ...
      files.push("../bl2.config.v2.js");
    }

    if(!window.BL2DB){
      files.push("../bl2.db.js");
    }
    ...
    if(!window.BDLV2Mirror){
      files.push("../patches/bdl.v2.mirror.js");
    }

    if(!window.BL2Backup){
      files.push("../bl2.backup.js");
    }

    if(!window.BL2Import){
      files.push("../bl2.import.js");
    }

    if(!window.BL2Sync){
      files.push("../bl2.sync.js");
    }

    if(!window.BL2Core){
      files.push("../bl2.core.js");
    }
  ...
  function ensureCoreReady(){
    return ensureCoreScripts().then(function(){
      ...
      if(
        window.BL2DB&&
        window.BL2Config&&
        Number(window.BL2Config.dbVersion||1)<2
      ){
        state.errors.push({
      ...
      if(
        ...
        typeof window.BDLOutboxBridge.install==="function"
      ){
        try{
          window.BDLOutboxBridge.install();
        }catch(error){}
      }

      if(
        window.BDLV2Mirror&&
        typeof window.BDLV2Mirror.install==="function"
      ){
        try{
          window.BDLV2Mirror.install();
        }catch(error2){}
      }

      if(
        bdlocal&&
        typeof bdlocal.ready==="function"
      ){
  ...
  function refreshKey(mode,options){
    options=options||{};
  ...
  function writeCachePayload(
    ...
    var payload={
      meta:{
        ...
        refreshMode:mode,
        refreshKey:refreshKey(mode,options),
        updatedAt:U.nowISO(),
        schemaVersion:
          window.BL2Config&&
        ...
        incremental:
          options.incrementalApplied===true,
        periodoId:U.canonicalPeriodId(
          options.periodoId||
          options.periodId||
          ""
      ...
      students:preferFresh(
        students,
        existing.students,
        allowEmpty
      ),
      requirements:preferFresh(
        requirements,
  ...
  function performLightRefresh(
    ...
    var periodPromise=
      typeof core.getPeriods==="function"
        ?core.getPeriods().catch(function(error){
          state.errors.push({
            file:"BL2Core.getPeriods",
            message:
              error&&error.message
                ?error.message
                :String(error),
  ...
  function performFullRefresh(
    ...
    var incremental=
      !!periodoId&&
      options.incremental!==false&&
      hasRows(existing.students);

    var periodPromise=
      typeof core.getPeriods==="function"
        ?core.getPeriods().catch(function(){
          return existing.periods||[];
        })
        :Promise.resolve(
          existing.periods||[]
    ...
    if(incremental){
      studentOptions.periodoId=periodoId;
      requirementOptions.periodoId=periodoId;
    }

    var studentPromise=
      typeof core.getStudents==="function"
        ?core.getStudents(studentOptions)
          .catch(function(error){
            state.errors.push({
              file:"BL2Core.getStudents",
              message:
                error&&error.message
                  ?error.message
                  :String(error),
    ...
    var requirementPromise=
      typeof core.getRequirements==="function"
        ?core.getRequirements(
          requirementOptions
        ).catch(function(error2){
          state.errors.push({
            file:"BL2Core.getRequirements",
            message:
    ...
    return Promise.all([
      ...
      var requirements=result[2]||[];

      if(incremental){
        students=replacePeriodRows(
          existing.students,
          students,
          periodoId
        );

        requirements=replacePeriodRows(
          existing.requirements,
          requirements,
          periodoId
        );

        state.metrics.incrementalExecuted+=1;
      }

      state.metrics.fullExecuted+=1;
      ...
      return writeCachePayload(
        ...
        Object.assign({},options,{
          incrementalApplied:incremental,
          allowEmpty:
            options.allowEmpty===true||
            options.changed===true
        })
  ...
  function firstActiveFull(){
    ...
    for(var i=0;i<keys.length;i+=1){
      if(
        keys[i].indexOf("full:")===0&&
        state.inFlight[keys[i]]
      ){
        return state.inFlight[keys[i]];
      }
    }
  ...
  function resolvePendingLightWith(promise){
  ...
  function enqueueRefresh(options){
    ...
    var activeFull=firstActiveFull();
    ...
    if(
      ...
      state.metrics.coalesced+=1;
      return state.inFlight["full:all"];
    }

    var cooldown=Math.max(
      0,
      Number(options.cooldown||700)
    );

    var lastAt=Number(
      state.lastCompleted[key]||0
    );

    var canSkip=
      options.force!==true&&
      options.changed!==true&&
      Date.now()-lastAt<cooldown;

    if(canSkip){
      state.metrics.cooldownSkipped+=1;
      return Promise.resolve(
        U.readCache()
      );
    }

    var operation=state.refreshSerial
      .catch(function(){
        return null;
      })
      .then(function(){
    ...
    state.refreshSerial=operation.catch(
      function(){
        return null;
      }
    );
    ...
    operation
      .then(function(){
        ...
        state.lastCompleted[key]=Date.now();
      })
      .catch(function(){
        delete state.inFlight[key];
        state.lastCompleted[key]=Date.now();
    ...
    if(mode==="full"){
      resolvePendingLightWith(operation);
    }

    return operation;
  }

  function mergeLightOptions(current,next){
  ...
  function ready(options){
    ...
    var shared=
      typeof U.requestSharedCache==="function"
        ?U.requestSharedCache({
          timeout:Number(
            options.sharedTimeout||1800
          )
        }).catch(function(){
          return U.readCache();
        })
    ...
    state.loading=shared
      .then(function(cache){
        ...
        return refreshCache({
          source:"BDLocalConexiones.ready",
          mode:
            U.hasData(cache)
              ?"light"
              :"full",
          light:U.hasData(cache),
          full:!U.hasData(cache),
          immediate:true,
          cooldown:0
        });
      })

...
- Evitar mostrar Base Local como pantalla activa independiente; cualquier llamada a BL abre Carga.
Con qué se conecta:
...
(function(window,document){
  ...
  var PRELOAD_ORDER=["tabla_principal","ficha_estudiante","stat_main","coordi","modulo_reporte","defart","titulacion"];
  var PRELOAD_DELAY_MS=2500;
  var PRELOAD_STEP_MS=1200;
  var PRELOAD_FLAG_KEY="REQ_MAQ_PRELOAD_ENABLED_V1";
  var BL_MODULES={tabla_principal:true,ficha_estudiante:true,stat_main:true,coordi:true,modulo_reporte:true,defart:true};
  ...
  function canonicalModuleId(moduloId){var raw=clean(moduloId);if(!raw)return "";if(raw==="baselocal")return "carga_excel";if(FALLBACK_MODULES[raw])return raw;var key=norm(raw).replace(/[_-]+/g," ");return MODULE_ALIASES[key]||raw;}
  ...
  function ensureBaseLocalReady(options){options=options||{};state.baseLocalReady=true;return true;}
  ...
  function boot(){var btn=document.getElementById("maq-btn-refresh");if(btn)btn.addEventListener("click",refrescarModuloActivo);var prev=document.getElementById("maq-btn-prev");if(prev)prev.addEventListener("click",pantallaAnterior);if(U.status)U.status("Modo rápido activo.");}

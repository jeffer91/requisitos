...
(function(window,document){
  ...
  function setFrameVisible(item,visible){
    ...
    if(visible){frame.hidden=false;frame.classList.remove("maq-frame-hidden");frame.style.display="block";frame.style.visibility="visible";frame.style.pointerEvents="auto";frame.setAttribute("aria-hidden","false");touch(item);}
  ...
  function makeFrame(module,options){
    ...
    frame.addEventListener("load",function(){var item=pool[module.id];if(item){item.loadedAt=Date.now();touch(item);}if(state.moduloActivoId===module.id){status("Pantalla activa: "+module.nombre);}emit("frame:loaded",{moduloId:module.id,modulo:module,preloaded:options.preload===true});});
  ...
  function navigate(moduleId){
    ...
    if(state.moduloActivoId===canonical){hideAll();setFrameVisible(item,true);label(module.nombre);emit("modulo:reabierto",{moduloId:canonical,modulo:module});enforceFrameLimit();return true;}
    ...
    hideAll();setFrameVisible(item,true);label(module.nombre);enforceFrameLimit();
  ...
  function boot(){var refreshButton=document.getElementById("maq-btn-refresh");if(refreshButton){refreshButton.addEventListener("click",refresh);}var previousButton=document.getElementById("maq-btn-prev");if(previousButton){previousButton.addEventListener("click",previous);}status("Modo rápido activo.");}

  window.MAQ_CORE={
    state:state,
    bus:{on:on,emit:emit},
    router:{navegarPorModuloId:navigate,pantallaAnterior:previous,canonicalModuleId:canonicalModuleId,buscarModulo:findModule},
    actions:{refrescarModuloActivo:refresh,ensureBaseLocalReady:ensureBaseLocalReady,schedulePreload:schedulePreload,evictFrame:removeFrame,enforceFrameLimit:enforceFrameLimit},

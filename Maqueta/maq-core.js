...
(function(window,document){
  ...
  function cloneModule(modulo){
    return modulo?{id:modulo.id,nombre:modulo.nombre,ruta:modulo.ruta,estado:modulo.estado}:null;
  }

  function canonicalModuleId(moduloId){
    var raw=clean(moduloId);
    if(!raw)return "";
    if(FALLBACK_MODULES[raw])return raw;
    var key=norm(raw).replace(/[_-]+/g," ");
    return MODULE_ALIASES[key]||raw;
  }

  function getRegistry(){
    return window.MAQ_MODULOS_REGISTRY||{};
  }

  function buscarModulo(moduloId){
    var canonical=canonicalModuleId(moduloId);
    var registry=getRegistry();
    var encontrado=null;

    try{
      if(registry&&typeof registry.buscarPorId==="function"){
        encontrado=registry.buscarPorId(canonical);
      }
    }catch(error){
      console.warn("[MAQ_CORE] Registro principal no disponible",error);
    }

    return encontrado||cloneModule(FALLBACK_MODULES[canonical]);
  }
  ...
  function emit(evt,payload){
    (listeners[evt]||[]).forEach(function(fn){
      try{
        ...
        console.error("[MAQ_CORE] Error",evt,e);
      }
    });
  }

  function host(){
    return document.getElementById("maq-main-frame-host");
  }

  function label(text){
    var el=document.getElementById("maq-current-module-label");
    if(el)el.textContent=text||"Sin módulo";
  }

  function setFrameVisible(item,visible){
    if(!item||!item.iframe)return;

    var iframe=item.iframe;

    if(visible){
      iframe.hidden=false;
      iframe.classList.remove("maq-frame-hidden");
      iframe.style.display="block";
      iframe.style.visibility="visible";
      iframe.style.pointerEvents="auto";
      iframe.setAttribute("aria-hidden","false");
    }else{
      iframe.classList.add("maq-frame-hidden");
      iframe.hidden=true;
  ...
  function hideAll(){
    Object.keys(pool).forEach(function(id){
      setFrameVisible(pool[id],false);
    });
  }

  function routeFor(modulo){
    if(!modulo)return "maq-pendiente.html";
    if(modulo.estado&&modulo.estado!=="activo"){
      return U.buildPendingUrl?U.buildPendingUrl(modulo):"maq-pendiente.html";
    }
    return modulo.ruta;
  }

  function shouldPrepareBaseLocal(moduloId){
    return !!BL_MODULES[canonicalModuleId(moduloId)];
  }

  function preloadAllowed(){
    try{
      return window.localStorage.getItem(PRELOAD_FLAG_KEY)==="true";
    }catch(error){
      return false;
    }
  }
  ...
  function makeFrame(modulo,options){
    ...
    if(shouldPrepareBaseLocal(modulo&&modulo.id)){
    ...
    iframe.className="maq-frame";
    iframe.title="Módulo: "+(modulo.nombre||modulo.id||"Requisitos");
    ...
    iframe.addEventListener("load",function(){
      if(state.moduloActivoId===modulo.id){
        if(U.status)U.status("Pantalla activa: "+modulo.nombre);
      }else if(options.preload===true){
        if(U.memory)U.memory("Precargada: "+modulo.nombre+" · "+Object.keys(pool).length+" pantalla(s)");
      }
    });

    iframe.addEventListener("error",function(){
      if(U.status)U.status("No se pudo cargar: "+(modulo.nombre||modulo.id));
    });

    h.appendChild(iframe);
    setFrameVisible({iframe:iframe},false);
  ...
  function ensureFrame(moduloId,options){
    var canonical=canonicalModuleId(moduloId);
    var modulo=buscarModulo(canonical);
    ...
    if(!pool[canonical]){
      pool[canonical]={
        iframe:makeFrame(modulo,options||{}),
        rutaBase:routeFor(modulo),
        nombre:modulo.nombre,
        estado:modulo.estado,
        preloaded:!!(options&&options.preload)
      };
    }
  ...
  function preloadNext(queue,index){
    if(index>=queue.length){
      ...
      if(U.memory)U.memory("Pantallas listas en memoria: "+Object.keys(pool).length);
      emit("preload:finished",{total:Object.keys(pool).length});
      return;
    }

    var id=canonicalModuleId(queue[index]);

    if(id&&id!==state.moduloActivoId&&!pool[id]){
      var item=ensureFrame(id,{preload:true});
      setFrameVisible(item,false);
    }

    setTimeout(function(){
      preloadNext(queue,index+1);
    },PRELOAD_STEP_MS);
  }
  ...
  function schedulePreload(){
    ...
    if(!state.preloadEnabled){
      state.preloadFinished=true;
      if(U.memory)U.memory("Precarga automática pausada · modo rápido activo");
      return;
    }
    ...
    setTimeout(function(){
      try{
        var available=PRELOAD_ORDER.filter(function(id){
          return !!buscarModulo(id);
        });
        preloadNext(available,0);
      }catch(error){
        console.warn("[MAQ_CORE] Precarga detenida",error);
      }
    },PRELOAD_DELAY_MS);
  }

  function saveNav(current,previous){
    if(!U.save||!U.NAV_KEYS)return;

    U.save(U.NAV_KEYS.ultimoModuloId,current||null);
    U.save(U.NAV_KEYS.anteriorModuloId,previous||null);

    if(U.saveNavState){
      U.saveNavState({
        ultimoModuloId:current||null,
        anteriorModuloId:previous||null
      });
    }
  }
  ...
  function navegarPorModuloId(moduloId){
    ...
    if(!modulo){
      console.error("[MAQ_CORE] Módulo no registrado:",moduloId);
      if(U.status)U.status("Módulo no registrado: "+moduloId);
      return;
    }

    if(shouldPrepareBaseLocal(canonical)){
      ensureBaseLocalReady();
    }

    var item=ensureFrame(canonical,{preload:false});

    if(!item||!item.iframe){
      if(U.status)U.status("No se pudo abrir: "+(modulo.nombre||canonical));
      return;
    }

    if(state.moduloActivoId===canonical){
      hideAll();
      setFrameVisible(item,true);
      label(modulo.nombre);
      schedulePreload();
      return;
    }
    ...
    state.moduloActivoId=canonical;

    saveNav(state.moduloActivoId,state.moduloAnteriorId);
    ...
    if(U.memory)U.memory("En memoria: "+Object.keys(pool).length+" pantalla(s)");
  ...
  function pantallaAnterior(){
    if(state.moduloAnteriorId){
      var prev=state.moduloAnteriorId;
      state.moduloAnteriorId=state.moduloActivoId;
      navegarPorModuloId(prev);
      return;
    }

    navegarPorModuloId(state.moduloInicialId);
  }

  function refrescarModuloActivo(){
    var id=state.moduloActivoId;
    var item=id?pool[id]:null;

    if(!item||!item.iframe)return;

    var sep=item.rutaBase.indexOf("?")>=0?"&":"?";
    item.iframe.src=item.rutaBase+sep+"_refresh="+Date.now();
  }

  function boot(){
    var btn=document.getElementById("maq-btn-refresh");
    if(btn)btn.addEventListener("click",refrescarModuloActivo);

    var prev=document.getElementById("maq-btn-prev");
    if(prev)prev.addEventListener("click",pantallaAnterior);

    if(U.status)U.status("Modo rápido activo.");
  }
  ...
  window.MAQ_CORE={
    ...
    router:{
      navegarPorModuloId:navegarPorModuloId,
      pantallaAnterior:pantallaAnterior,
      canonicalModuleId:canonicalModuleId,
      buscarModulo:buscarModulo
    },
    actions:{
      refrescarModuloActivo:refrescarModuloActivo,
      ensureBaseLocalReady:ensureBaseLocalReady,
      schedulePreload:schedulePreload
    },
    performance:{
      preloadFlagKey:PRELOAD_FLAG_KEY,
      preloadAllowed:preloadAllowed,
      shouldPrepareBaseLocal:shouldPrepareBaseLocal
    }
  };
})(window,document);
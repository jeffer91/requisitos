...
- Recuperar rutas internas aunque el registro principal no haya cargado todavía.
- Normalizar nombres antiguos o escritos con espacios para no dejar pantallas en blanco.
Con qué se conecta:
...
(function(window,document){
  ...
  var FALLBACK_MODULES={
    carga_excel:{id:"carga_excel",nombre:"Carga",ruta:"../Gestion/Excel/excel.html",estado:"activo"},
    baselocal:{id:"baselocal",nombre:"Base Local",ruta:"../BaseLocal/baselocal.html",estado:"activo"},
  ...
  function buscarModulo(moduloId){
    ...
    var encontrado=null;
    try{if(registry&&typeof registry.buscarPorId==="function"){encontrado=registry.buscarPorId(canonical);}}catch(error){console.warn("[MAQ_CORE] Registro principal no disponible",error);}
  ...
  function shouldPrepareBaseLocal(moduloId){return !!BL_MODULES[canonicalModuleId(moduloId)];}
  ...
  function makeFrame(modulo, options){
    ...
    iframe.addEventListener("error",function(){if(U.status)U.status("No se pudo cargar: "+(modulo.nombre||modulo.id));});
  ...
  function ensureFrame(moduloId, options){
    ...
    if(!pool[canonical]){pool[canonical]={iframe:makeFrame(modulo,options||{}),rutaBase:routeFor(modulo),nombre:modulo.nombre,estado:modulo.estado,preloaded:!!(options&&options.preload)};}
  ...
  function schedulePreload(){
    ...
    setTimeout(function(){
      try{
        var available=PRELOAD_ORDER.filter(function(id){return !!buscarModulo(id);});
  ...
  function navegarPorModuloId(moduloId){
    ...
    if(state.moduloActivoId===canonical){label(modulo.nombre);schedulePreload();return;}
    ...
    ensureFrame(canonical,{preload:false});
    hideAll();
    if(pool[canonical]&&pool[canonical].iframe)pool[canonical].iframe.classList.remove("maq-frame-hidden");
    label(modulo.nombre);
    if(U.memory)U.memory("En memoria: "+Object.keys(pool).length+" pantalla(s)"+(state.baseLocalReady?" + Base Local":""));
    emit("modulo:cambiado",{moduloId:canonical,modulo:modulo,anteriorModuloId:state.moduloAnteriorId});
    schedulePreload();
  }

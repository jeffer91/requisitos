...
- Respetar el estado manual ACTIVO/RETIRADO definido en Ficha.
========================================================= */
(function(window){
  "use strict";
  ...
  function firstValue(row,fields){
    ...
    var wanted=(fields||[]).map(key),keys=Object.keys(row);
  ...
  function preserveManualEnrollment(con,rows,period,options){
    if(options&&options.overrideManualEnrollment===true){return Promise.resolve(rows);}
    ...
    return Promise.resolve(con.listStudents({periodoId:period.periodoId,periodId:period.periodoId,matricula:"",limit:0})).then(function(existing){
      ...
      return rows.map(function(input){
        ...
        if(!manualEnrollment(previous)){return row;}
        ["estadoMatricula","retirado","retiradoEn","reactivadoEn","estadoMatriculaActualizadaEn","estadoMatriculaManual","estadoMatriculaManualActualizadaEn","estadoMatriculaManualOrigen"].forEach(function(field){if(previous[field]!==undefined){row[field]=previous[field];}});
  ...
  function executeSave(normalized,validation,options){
    ...
    var rows=rowsFromNormalized(normalized),period=selectedPeriod(normalized,options);
    ...
    return ensureConnector().then(function(con){
      return preserveManualEnrollment(con,prepared,period,options).then(function(finalRows){
        prepared=finalRows;
    ...
    }).then(function(result){var finalResult=normalizeResult(result,period,prepared);emit("bdlocal:changes-created",{source:"ConCarga",localOnly:true,manualCloudSync:true,periodoId:period.periodoId,total:Array.isArray(finalResult.changes)?finalResult.changes.length:Number(finalResult.changes||0),changes:Array.isArray(finalResult.changes)?finalResult.changes:[],targets:["firebase","supabase","google"],at:new Date().toISOString()});emit("bdlocal:carga-save-finish",{ok:finalResult.ok,engine:"ConCarga",total:finalResult.total,saved:finalResult.saved,updated:finalResult.updated,merged:finalResult.merged,periodoId:period.periodoId,periodoLabel:period.periodoLabel,at:new Date().toISOString()});return finalResult;}).catch(function(error){emit("bdlocal:carga-save-error",{ok:false,source:"ConCarga",error:error.message||String(error),periodoId:period.periodoId,at:new Date().toISOString()});throw error;});
  }
  ...
  window.CargaSave={version:"2.1.0-manual-enrollment-lock",save:save,isSaving:function(){return !!activeSave;},helpers:{selectedPeriod:selectedPeriod,injectPeriod:injectPeriod,normalizeCedula:normalizeCedula,validateAnalysis:validateAnalysis,ensureDependencies:ensureConnector,preserveManualEnrollment:preserveManualEnrollment}};
})(window);

...
(function(window){
  ...
  function clone(value){if(Array.isArray(value)){return value;}if(!value||typeof value!=="object"){return value;}try{return JSON.parse(JSON.stringify(value));}catch(error){return value;}}
  ...
  function fieldName(row,fields){row=row||{};var wanted=(fields||[]).map(fieldKey),keys=Object.keys(row);for(var i=0;i<keys.length;i+=1){if(wanted.indexOf(fieldKey(keys[i]))>=0){return keys[i];}}return "";}
  ...
  function selectedPeriod(normalized,options){normalized=normalized||{};options=options||{};var detected=normalized.periodoDetectado||{},local=localPeriod(),id=canonicalPeriodId(options.periodoCanonicoId||options.periodoId||detected.periodoCanonicoId||detected.periodoId||detected.id||local.periodoId||""),label=text(options.periodoCanonicoLabel||options.periodoLabel||detected.periodoCanonicoLabel||detected.periodoLabel||detected.label||local.periodoLabel||id);return {id:id,label:label||id,periodoId:id,periodoLabel:label||id,periodoCanonicoId:id,periodoCanonicoLabel:label||id,valid:!!id&&id!=="SIN_PERIODO"};}
  ...
  function copyMissingGroup(row,previous,group){var canonical=group[0],aliases=group[1];if(hasField(row,aliases)){return;}var value=firstValue(previous,aliases.concat([canonical]));if(value!==undefined&&value!==null&&text(value)!==""){row[canonical]=clone(value);}}
  function preserveBlankPersonalEmail(row,previous){var aliases=["CorreoPersonal","correoPersonal","correo_personal"],incomingName=fieldName(row,aliases);if(!incomingName||text(row[incomingName])!==""){return;}var previousValue=firstValue(previous,aliases);if(text(previousValue)===""){return;}row[incomingName]=previousValue;row.CorreoPersonal=previousValue;row._correoPersonalPreservado=true;}
  function preserveExistingFields(con,rows,period,options){
    ...
    return Promise.resolve(con.listStudents({periodoId:period.periodoId,periodId:period.periodoId,matricula:"",limit:0})).then(function(existing){
      existing=Array.isArray(existing)?existing:(existing&&Array.isArray(existing.rows)?existing.rows:[]);var index=Object.create(null);existing.forEach(function(row){var id=cedulaOf(row);if(id){index[id]=row;}});
      return rows.map(function(input){var row=Object.assign({},input||{}),previous=index[cedulaOf(row)];if(!previous){return row;}if(!(options&&options.overrideManualEnrollment===true)&&manualEnrollment(previous)){["estadoMatricula","retirado","retiradoEn","reactivadoEn","estadoMatriculaActualizadaEn","estadoMatriculaManual","estadoMatriculaManualActualizadaEn","estadoMatriculaManualOrigen"].forEach(function(field){if(previous[field]!==undefined){row[field]=clone(previous[field]);}});row._estadoMatriculaManualPreservado=true;}preserveBlankPersonalEmail(row,previous);REQUIREMENT_GROUPS.concat(CONTACT_GROUPS,NOTE_GROUPS).forEach(function(group){copyMissingGroup(row,previous,group);});row._camposAusentesPreservados=true;return row;});
    });
  }
  ...
  function executeSave(normalized,validation,options){
    ...
    return ensureConnector().then(function(con){activeConnector=con;progress(15,"Protegiendo información existente");return preserveExistingFields(con,prepared,period,options).then(function(finalRows){prepared=finalRows;progress(35,"Guardando estudiantes en BDLocal");var task=typeof con.saveStudents==="function"?con.saveStudents(prepared,Object.assign({},options,period,{normalized:true,source:options.source||"carga_excel",fileName:normalized.fileName||"",origen:normalized.origen||"",markRetired:options.markRetired===true})):con.guardarEstudiantes(prepared,period,options);return Promise.resolve(task);});}).then(function(result){
      ...
      return registerImport(activeConnector,record,finalResult).catch(function(error){finalResult.ok=true;finalResult.auditOk=false;finalResult.auditError=text(error&&error.message||error);finalResult.auditQueued=queueAuditFallback(record,error);finalResult.message="Los estudiantes se guardaron en BDLocal, pero el registro de auditoría quedó pendiente.";emit("bdlocal:carga-audit-pending",{ok:false,periodoId:period.periodoId,error:finalResult.auditError,queued:finalResult.auditQueued,record:record,at:new Date().toISOString()});}).then(function(){return finalResult;});
    }).then(function(finalResult){progress(100,finalResult.auditOk===false?"Guardado completo; auditoría pendiente":"Carga guardada y auditada");emit("bdlocal:changes-created",{source:"ConCarga",localOnly:true,manualCloudSync:true,periodoId:period.periodoId,total:Array.isArray(finalResult.changes)?finalResult.changes.length:Number(finalResult.changes||0),changes:Array.isArray(finalResult.changes)?finalResult.changes:[],targets:["firebase","supabase","google"],importacionId:finalResult.importacionId||"",auditOk:finalResult.auditOk!==false,at:new Date().toISOString()});emit("bdlocal:carga-save-finish",{ok:finalResult.ok,engine:"ConCarga",total:finalResult.total,saved:finalResult.saved,updated:finalResult.updated,merged:finalResult.merged,periodoId:period.periodoId,periodoLabel:period.periodoLabel,importacionId:finalResult.importacionId||"",auditOk:finalResult.auditOk!==false,at:new Date().toISOString()});return finalResult;}).catch(function(error){emit("bdlocal:carga-save-error",{ok:false,source:"ConCarga",error:error.message||String(error),periodoId:period.periodoId,at:new Date().toISOString()});throw error;});
  }

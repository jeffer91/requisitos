...
- Detectar XLS antiguo guardado como HTML y reparar codificación.
========================================================= */
(function(window){
  "use strict";

  var VERSION="4.0.0-fast-full-reader";
  ...
  function readBuffer(blob,progress){
    return new Promise(function(resolve,reject){
      ...
      reader.onprogress=function(event){
        if(progress&&event.lengthComputable){progress(event.loaded,event.total);}
      };
  ...
  function readAll(file){
    return readBuffer(file,function(loaded,total){emit(5+(loaded/Math.max(1,total))*40,"Leyendo archivo: "+Math.round(loaded/1048576)+" MB","reading");});
  }
  function decode(buffer,encoding){
    try{return new TextDecoder(encoding||"utf-8").decode(buffer);}catch(error){return new TextDecoder("utf-8").decode(buffer);}
  }
  ...
  function decodeScore(value){
    var sample=String(value||"").slice(0,131072);
    return countMatches(sample,/\uFFFD/g)*1000+countMatches(sample,/Ã.|Â.|â.|�/g)*80-countMatches(sample,/[áéíóúÁÉÍÓÚñÑüÜ]/g)*4;
  }
  function bestDecodedText(buffer){
    ...
    [
      ...
      var value=decode(buffer,item.encoding),score=decodeScore(value);
      if(score<best.score){best={text:value,encoding:item.label,score:score};}
    });
    return best;
  }
  ...
  function looksLikeHtml(source){
    source=String(source||"").slice(0,12000).toLowerCase();
    return source.indexOf("<!doctype html")>=0||source.indexOf("<html")>=0||source.indexOf("<table")>=0||source.indexOf("urn:schemas-microsoft-com:office:excel")>=0||source.indexOf("mso-")>=0&&source.indexOf("<td")>=0;
  }
  ...
  function safeRows(rows){
    return (Array.isArray(rows)?rows:[]).map(function(input){
      ...
      Object.keys(input||{}).forEach(function(key){
        ...
        if(lowerKey!=="__proto__"&&lowerKey!=="prototype"&&lowerKey!=="constructor"){row[String(key).trim()]=input[key];}
      });
      return row;
    });
  }
  function jsonRows(data){
    ...
    if(data&&data.tables&&Array.isArray(data.tables.estudiantes)){return data.tables.estudiantes;}
    return [];
  }
  function parseJson(source,fileName){
    emit(60,"Interpretando JSON...","reading");
    var rows=safeRows(jsonRows(JSON.parse(source)));
    emit(90,"JSON leído: "+rows.length+" filas","reading");
    return {rows:rows,fileName:fileName,origen:"json",detectedType:"json",warnings:rows.length?[]:[{tipo:"JSON_SIN_FILAS",mensaje:"El JSON no contiene filas reconocibles."}]};
  }
  function parseText(source,fileName,type,encoding,warnings){
    warnings=Array.isArray(warnings)?warnings:[];
    var reader=window.CargaReaderTXT;
  ...
  function parseCsv(source,fileName,encoding,warnings){
    warnings=Array.isArray(warnings)?warnings:[];
    var reader=window.CargaReaderCSV;
    var task=reader&&typeof reader.parseAsync==="function"?reader.parseAsync(source||""):Promise.resolve(reader&&typeof reader.parse==="function"?reader.parse(source||""):[]);
  ...
  function parseHtml(source,fileName,encoding,warnings){
    ...
    return new Promise(function(resolve,reject){
      window.setTimeout(function(){
        try{
          var result=window.CargaReaderHTML.parse(source||"",{fileName:fileName,encoding:encoding||""})||{};
          result.rows=safeRows(result.rows);
          result.fileName=fileName;result.origen="html";result.detectedType="html";result.encoding=encoding||"";result.warnings=warnings.concat(result.warnings||[]);
          emit(90,"HTML leído: "+result.rows.length+" filas","reading");resolve(result);
  ...
  function readTextFile(file,type,extraWarnings){
    return readAll(file).then(function(buffer){
      ...
      if(type==="json"||looksLikeJson(source)){
        try{return Promise.resolve(parseJson(source,file.name||"archivo"));}
        catch(error){warnings.push({tipo:"JSON_INVALIDO",mensaje:"El archivo parecía JSON, pero no pudo interpretarse."});}
      }
  ...
  function read(file){
    ...
    var ext=extOf(file),fileName=file.name||"archivo";
    emit(1,"Detectando formato de "+fileName+"...","reading");
    return probe(file).then(function(sampleBuffer){
      var sampleDecoded=bestDecodedText(sampleBuffer),sampleText=sampleDecoded.text||"";
      if((ext==="xls"||ext==="xlsx")&&looksLikeHtml(sampleText)){
        return readTextFile(file,"html",[{tipo:"XLS_HTML_DETECTADO",mensaje:"El archivo parece ser XLS antiguo guardado como HTML. Se leyó como tabla HTML."}]);
      }
      if(ext==="xlsx"||ext==="xls"||isZipXlsx(sampleBuffer)||isOleXls(sampleBuffer)){
        return readExcel(file).catch(function(error){
          return readTextFile(file,ext||"txt",[{tipo:"XLSX_FALLBACK_TEXTO",mensaje:"No se pudo leer como Excel normal. Se intentó como texto. Detalle: "+(error.message||error)}]);
        });
      }
  ...
  window.CargaReaderFile={version:VERSION,read:read,helpers:{extOf:extOf,bestDecodedText:bestDecodedText,looksLikeHtml:looksLikeHtml,isZipXlsx:isZipXlsx,isOleXls:isOleXls}};
})(window);

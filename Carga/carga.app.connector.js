...
- Evitar recálculos completos al actualizar los controles de la pantalla.
...
(function(window){
  ...
  function clone(value){if(value&&Array.isArray(value.rowsMapeadas)){return value;}try{return JSON.parse(JSON.stringify(value));}catch(error){return value;}}
  function emit(name,detail){try{window.dispatchEvent(new CustomEvent(name,{detail:detail||{}}));}catch(error){}}
  function progress(percent,message,phase){if(state&&typeof state.patch==="function"){state.patch({progress:{current:Number(percent)||0,total:100,message:message||""}});}emit("carga:progress",{percent:Math.max(0,Math.min(100,Number(percent)||0)),message:message||"",phase:phase||"analysis"});}
  ...
  function signature(periodoId,cedulas){var hash=2166136261;hash=fnvUpdate(hash,canon(periodoId));(cedulas||[]).forEach(function(id){hash=fnvUpdate(hash,"|");hash=fnvUpdate(hash,id);});return canon(periodoId)+":"+(hash>>>0).toString(16)+":"+(cedulas||[]).length;}
  function invalidateAnalysis(){approvedGuard=null;approvedNormalized=null;}
  ...
  function readFile(file,options){if(!file){return Promise.reject(new Error("Selecciona un archivo."));}options=Object.assign({},periodInfo(options||{}),options||{},{fileName:file.name||""});state.reset();invalidateAnalysis();state.setStatus(cfg.estados.reading,"Leyendo archivo");progress(0,"Preparando archivo","reading");return requireModule("CargaReaderFile","read").read(file).then(function(result){result=result||{};return processRows(result.rows||[],Object.assign({},options,result,{origen:result.origen||"archivo",fileName:result.fileName||options.fileName,readerWarnings:result.warnings||[]}));}).catch(function(error){state.setStatus(cfg.estados.error,error.message||"No se pudo leer el archivo");progress(0,error.message||"No se pudo leer el archivo","error");throw error;});}
  ...
  function canSave(period){var info=periodInfo(period),current=state.get();return !!(approvedGuard&&approvedGuard.ok&&approvedGuard.periodoId===info.id&&current.normalized===approvedNormalized);}

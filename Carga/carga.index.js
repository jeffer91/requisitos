...
- Instalar una autoridad única de divisiones por período.
...
(function(window,document){
  ...
  var DIVISION_AUTHORITY_PATH="./carga.divisiones.authority.js";
  ...
  function ensureConnector(){
    ...
    connectorTask=ensureAdapter()
      ...
      .then(function(con){
        return loadScript(DIVISION_AUTHORITY_PATH,function(){return window.CargaDivisionAuthority;}).then(function(authority){
          return Promise.resolve(authority&&typeof authority.install==="function"?authority.install():con).then(function(){return con;});
        });
      })
      .then(function(con){
  ...
  function boot(){
    ensureConnector().then(function(con){
      emit("carga:bdlocal-ready",{ok:true,source:"ConCarga",status:typeof con.status==="function"?con.status():{},divisionAuthority:con.divisionAuthorityVersion||"",at:new Date().toISOString()});
      emit("carga:connection-ready",{ok:true,source:"ConCarga",app:window.CargaApp&&window.CargaApp.version||"",ui:window.CargaUI&&window.CargaUI.version||"",divisionAuthority:con.divisionAuthorityVersion||"",at:new Date().toISOString()});
      return retryAudits();
  ...
  window.CargaConnectionIndex={version:"4.4.0-division-authority",ensureConnector:ensureConnector,refreshPeriods:refreshPeriods,retryAudits:retryAudits};

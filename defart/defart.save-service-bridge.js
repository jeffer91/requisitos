...
(function(window){
  ...
  function num(value){
    ...
    return Number.isFinite(number)
      ?Math.max(0,Math.min(10,Math.round(number*100)/100))
      :null;
  }
  function firstValue(){
  ...
  function stateRows(){
    try{
      ...
      var data=state.data||{};
      return Array.isArray(data.exportRows)&&data.exportRows.length
        ?data.exportRows
        :(Array.isArray(data.rows)?data.rows:[]);
    }catch(error){return [];}
  }
  function rowId(row){
    return text(row&&(row._defId||row.idEstudiantePeriodo||row.studentId||row._docId||row.id||row.cedula));
  }
  ...
  function notaFromChange(change){
    ...
    var article=Object.prototype.hasOwnProperty.call(change,"nart")
      ?num(change.nart)
      :num(firstValue(row._nart,row.Notart,row.notart,row.Nart,row.nart));
    var defense=Object.prototype.hasOwnProperty.call(change,"ndef")
      ?num(change.ndef)
      :num(firstValue(row._ndef,row.Notdef,row.notdef,row.Ndef,row.ndef));
  ...
  function saveDirect(changesList){
    ...
    return chain.then(function(){
      var result={
        ...
        direct:true,
        source:"ConDefart",
        message:saved+" cambio(s) guardado(s) mediante ConDefart."
      };
  ...
  window.DefartSaveServiceBridge={
    ...
    splitCanonicalId:splitCanonicalId
  };
  install();
})(window);
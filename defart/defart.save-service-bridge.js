...
(function(window){
  ...
  function num(value){var raw=text(value).replace(",",".");if(!raw){return null;}var number=Number(raw);return Number.isFinite(number)?Math.max(0,Math.min(10,Math.round(number*100)/100)):null;}
  ...
  function notaFromChange(change){
    change=change||{};var row=findRow(change)||{},id=text(row.idEstudiantePeriodo||row.studentId||row._docId||row._defId||change.id),parts=splitCanonicalId(id);
    var periodoId=text(row._periodoId||row.periodoId||row.periodId||parts.periodoId),cedula=text(row._cedula||row.cedula||row.numeroIdentificacion||parts.cedula);
    ...
    return {idEstudiantePeriodo:id,studentId:id,periodoId:periodoId,periodId:periodoId,cedula:cedula,numeroIdentificacion:cedula,Notart:article,Notdef:defense,Notafinal:finalGrade,Nart:article,Ndef:defense,Nfinal:finalGrade,notart:article,notdef:defense,notafinal:finalGrade,estadoNota:finalGrade==null?"PENDIENTE":(finalGrade>=7?"APROBADO":"NO_APROBADO"),origen:"defart",updatedAt:new Date().toISOString()};
  }
  function saveDirect(changesList){
    ...
    if(!current||typeof current.save!=="function"){return Promise.reject(new Error("ConDefart.save no está disponible."));}

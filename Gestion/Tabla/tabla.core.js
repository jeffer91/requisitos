...
- Recuperar períodos desde las cachés disponibles.
- Reintentar la lectura general de estudiantes cuando una consulta directa
  por período devuelve vacío.
- Comparar estudiantes y períodos usando ID, etiqueta y alias conocidos.
Con qué se conecta:
...
(function(window){
  ...
  var VERSION="2.5.0-period-students-fallback";
  ...
  var ALIAS={
    ...
    pp:"practicasvinculacion",
    practicas:"practicasvinculacion",
    practicasvinculacion:"practicasvinculacion",

    vinc:"vinculacion",
    vinculacion:"vinculacion",

    grad:"seguimientograduados",
    graduados:"seguimientograduados",
    seguimientograduados:"seguimientograduados",

    ing:"ingles",
    ingles:"ingles",
    segundalengua:"ingles",

    datos:"actualizaciondatos",
    actualizaciondatos:"actualizaciondatos",

    nart:"nota_articulo",
    notaarticulo:"nota_articulo",
    nota_articulo:"nota_articulo",

    ndef:"nota_defensa",
    notadefensa:"nota_defensa",
    nota_defensa:"nota_defensa",

    sinarticulo:"sin_articulo",
    sin_articulo:"sin_articulo",

    noaprueba:"no_aprueba",
    no_aprueba:"no_aprueba"
  };
  ...
  function repo(){
    if(!window.ExcelLocalRepo){
      throw new Error(
        "ExcelLocalRepo no disponible. Primero carga Base Local."
      );
    }
  ...
  function hasCore(){
    return !!(
      dataEngine()&&
      typeof dataEngine().listStudents==="function"
    );
  }
  ...
  function hasBL2Repo(){
    return !!(
      bl2Repo()&&
      typeof bl2Repo().buscar==="function"
    );
  }
  ...
  function compactMap(row){
    ...
    if(
      row._tablaCompactMap&&
      typeof row._tablaCompactMap==="object"
    ){
  ...
  function telegramUrl(row){
    ...
    if(info.chatId){
      return (
        "tg://user?id="+
        encodeURIComponent(info.chatId)
      );
    }
  ...
  function valueOf(row,key){
    ...
    try{
      if(
        reqEngine()&&
        typeof reqEngine().valueOf==="function"
      ){
        value=reqEngine().valueOf(
          row,
          key
        );
    ...
    try{
      if(
        normalizer()&&
        typeof normalizer().value==="function"
      ){
  ...
  function estadoCelda(value){
    try{
      if(
        reqEngine()&&
        typeof reqEngine().cellStatus==="function"
      ){
        var external=
          reqEngine().cellStatus(value);
  ...
  function applicableRequirements(row){
    ...
    try{
      if(
        reqEngine()&&
        typeof reqEngine()
          .requirementsForStudent==="function"
      ){
        list=
          reqEngine()
            .requirementsForStudent(row||{})||
          [];
      }
    }catch(error){
      list=[];
  ...
  function noteFilterMatches(row,key){
    ...
    if(key==="nota_articulo"){
      return !!info.faltaArticulo;
    }

    if(key==="nota_defensa"){
      return !!info.faltaDefensa;
    }

    if(key==="sin_articulo"){
      return !!info.sinArticulo;
    }

    if(key==="no_aprueba"){
      return !!info.noAprueba;
    }
  ...
  function matchesRequirementFilters(
    row,
    filters
  ){
  ...
  function wantedPeriodTokens(wanted){
    var out=periodTokens(wanted);
    ...
    known.forEach(function(period){
      ...
      var related=
        candidates.some(function(candidate){
          return out.some(function(current){
            return sameToken(
              candidate,
              current
            );
  ...
  function rowMatchesPeriod(row,wanted){
    ...
    var wantedTokens=
      wantedPeriodTokens(wanted);

    return rowTokens.some(function(rowToken){
      return wantedTokens.some(function(wantedToken){
        return samePeriod(
          rowToken,
          wantedToken
        );
  ...
  function abreviarCarrera(value){
    ...
    if(
      key.indexOf("administr")>=0
    ){
      return prefix+" Admin";
    }

    if(
      key.indexOf("enfermer")>=0
    ){
      return prefix+" Enf";
    }

    if(
      key.indexOf("software")>=0||
      key.indexOf("desarrollo")>=0
    ){
      return prefix+" Software";
    }

    if(
      key.indexOf("marketing")>=0||
      key.indexOf("mercad")>=0
    ){
      return prefix+" Mkt";
    }

    if(
      key.indexOf("alimento")>=0
    ){
      return prefix+" Alimentos";
    }

    if(
      key.indexOf("talento")>=0||
      key.indexOf("humano")>=0
    ){
      return prefix+" Tal. Hum";
    }
    ...
    if(
      key.indexOf("electron")>=0
    ){
      return prefix+" Electr";
    }
  ...
  function decorate(row){
    if(
      row&&
      row._tablaCoreDecoratedVersion===VERSION
    ){
      return row;
    }
  ...
  function baseRows(opts){
    ...
    try{
      ...
      try{
        console.warn(
          "[TablaCore] BL2EstudiantesRepo falló",
          error2
        );
      }catch(ignore2){}
    }
    ...
    try{
      rows=load(rowsFromExcel);
    }catch(error3){
      rows=[];

      try{
        console.warn(
          "[TablaCore] ExcelLocalRepo falló",
          error3
        );
      }catch(ignore3){}
    }
    ...
    var rows=
      baseRows(opts)
        .filter(function(row){
          if(
            matricula&&
            row._estadoMatricula!==matricula
          ){
            return false;
          }
  ...
  function summary(list){
    ...
    if(
      summaryMemo&&
      summaryMemo.has(list)
    ){
      return Object.assign(
        {},
        summaryMemo.get(list)
      );
    }
  ...
  window.TablaCore={
    ...
    _ensureRequirementState:
      ensureRequirementState
  };
})(window);